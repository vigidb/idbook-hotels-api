...
class UserManager(BaseUserManager):
    ...
    def create_customer(self, email, mobile_number, password=None):
        user = self.create_user(
            email, mobile_number, password=password, is_customer=True
        )
        return user

    def create_shop_owner(self, email, mobile_number, password=None):
        user = self.create_user(
            email, mobile_number, password=password, is_shop_owner=True
        )
        return user

    def create_stylist(self, email, mobile_number, password=None):
        user = self.create_user(
            email, mobile_number, password=password, is_stylist=True
        )
        return user

    def create_agent(self, email, mobile_number, password=None):
        user = self.create_user(email, mobile_number, password=password, is_agent=True)
        return user

    def create_manager(self, email, mobile_number, password=None):
        user = self.create_user(
            email, mobile_number, password=password, is_manager=True
        )
        return user
...
class User(AbstractBaseUser):
    ...
    education = models.CharField(
        max_length=25, choices=EDUCATION_CHOICES, default="Others"
    )
    ...
    shop_owner = models.BooleanField(
        default=False
    )  # if paid verified manually by admin
...
class FCMToken(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, null=True, related_name="fcm_user"
    )
    fcm_token = models.CharField(max_length=200, unique=True)
    token_type = models.CharField(
        max_length=10, choices=FCM_TOKEN_CHOICE, default="android"
    )
    user_type = models.CharField(
        max_length=25, choices=USER_TYPE_CHOICE, default="customer"
    )
...
class ShopDetail(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="shop_owner_user"
    )
    ...
    service_category = models.CharField(
        max_length=255, choices=SERVICE_CATEGORY_TYPE_CHOICES, default=""
    )
    ...
    location = models.CharField(
        max_length=255, blank=True, null=True, help_text="Google map url"
    )
    area_name = models.CharField(
        max_length=60, null=True, blank=True, help_text="Area Name"
    )
    city_name = models.CharField(
        max_length=35, null=True, blank=True, help_text="City Name"
    )
...
class Gallery(models.Model):
    shop = models.ForeignKey(
        ShopDetail, on_delete=models.CASCADE, related_name="gallery"
    )
...
class WorkingDay(models.Model):
    ...
    def __str__(self):
        return "{} is {} for {}".format(
            self.day_name, "working" if self.is_working else "not working", self.shop
        )
...
class Stylist(models.Model):
    shop = models.ForeignKey(
        ShopDetail, on_delete=models.CASCADE, related_name="stylist"
    )
    mobile_number = models.CharField(
        max_length=10,
        db_index=True,
...
class Service(models.Model):
    shop = models.ForeignKey(
        ShopDetail, on_delete=models.CASCADE, related_name="shop_services"
    )
    service_type = models.CharField(
        max_length=25, choices=SERVICE_TYPE_CHOICES, default="Others"
    )
...
class Appointment(models.Model):
    stylist = models.ForeignKey(
        Stylist, on_delete=models.CASCADE, related_name="stylist_appointment"
    )
    user = models.ForeignKey(
        User,
    ...
    status = models.CharField(
        max_length=20, choices=APPOINTMENT_STATUS, default="Waiting"
    )
...
class KYCDocument(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="kyc_document"
    )
    document_type = models.CharField(
        max_length=15, choices=KYC_DOCUMENT_CHOICES, default="Others"
    )
    ...
    status = models.CharField(
        max_length=10, choices=KYC_STATUS_CHOICES, default="Others"
    )
...
class PayoutCalculation(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="payout_calculation_user"
    )
...
class Banners(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="banners_user"
    )
    upload_banner = models.FileField(
        upload_to="banners",
        blank=True,
        validators=[FileExtensionValidator(allowed_extensions=["png", "jpeg", "jpg"])],
        help_text="Max size 2MB, and only pdf are allowed",
...
class Withdrawal(models.Model):
    ...
    txn_type = models.CharField(
        max_length=20, choices=TXN_TYPE_CHOICES, default="Others"
    )
    info = models.CharField(max_length=15, blank=True, null=True)
    method = models.CharField(
        max_length=20, choices=PAYMENT_METHOD_CHOICES, default="Others"
    )
    status = models.CharField(
        max_length=10, choices=PAYMENT_STATUS_CHOICES, default="Others"
    )
...
class Enquiry(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_enquiry"
    )
    replied_by = models.ForeignKey(
        User, on_delete=models.CASCADE, blank=True, null=True, related_name="user_reply"
    )
...
class Hotel(models.Model):
    ...
    manager_gender = models.CharField(
...
class Room(models.Model):
    ...
    TIME_SLOTS = (("4 HOURS", 4), ("8 HOURS", 8), ("12 HOURS", 12), ("24 HOURS", 24))
