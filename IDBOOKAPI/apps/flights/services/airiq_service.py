...
class AirIQService:
    ...
    def _make_request(
        self, endpoint: str, data: dict, method: str = "POST", booking=None
    ) -> Tuple[dict, bool]:
        """
        ...
        if "Login" not in endpoint:
            ...
            logger.warning(
                "_make_request called for login endpoint - should use _perform_login instead"
            )
        ...
        try:
            ...
            if token_timeout and "Login" not in endpoint:
                logger.warning(
                    "Detected AirIQ token timeout - refreshing token and retrying once"
                )
                # Invalidate cached token
                try:
                    AirIQTokenCache.objects.filter(is_active=True).update(
                        is_active=False
                    )
                except Exception:
                    pass
    ...
    def _should_refresh_token(self) -> bool:
        ...
        if token_created_date < current_date:
            logger.info(
                f"Token created on {token_created_date}, current date is {current_date} - daily refresh needed"
            )
            return True
        ...
        if cached_token_record.expires_at <= now + timedelta(hours=2):
            logger.info(
                f"Token expires at {cached_token_record.expires_at}, within 2 hours - proactive refresh"
            )
            return True
    ...
    def _perform_login(self) -> Tuple[dict, bool]:
        ...
        try:
            logger.info(
                f"Attempting AirIQ login with Basic Auth for agent: {self.agent_id}"
            )
            ...
            if isinstance(response_data, dict):
                ...
                if "Status" in response_data:
                    ...
                    logger.error(
                        f"Login failed - No token in response: {response_data}"
                    )
            ...
            if is_success:
                logger.info(
                    f"AirIQ login successful (response time: {response_time_ms}ms)"
                )
            else:
                logger.error(
                    f"AirIQ login failed - Code: {result_code}, Error: {error_message}"
                )
    ...
    def authenticate(self) -> bool:
        ...
        if not self._should_refresh_token():
            ...
            if cached_token:
                ...
                if cached_token_record:
                    ...
                    logger.info(
                        f"Using cached AirIQ token (expires: {self._token_expires_at})"
                    )
                    return True
        ...
        if is_success and "Token" in response_data:
            ...
            token_cache = AirIQTokenCache.cache_token(
                self._auth_token, expires_at=today_end
            )

            logger.info(
                f"AirIQ authentication successful - new token cached (expires: {today_end})"
            )
    ...
    def get_token_status(cls) -> dict:
        ...
        if active_token:
            ...
            return {
                ...
                "needs_refresh": active_token.created_at.date() < now.date()
                or hours_until_expiry < 2,
            }
        else:
            return {"has_valid_token": False, "needs_refresh": True}
    ...
    def search_flights(self, search_params: dict) -> Tuple[dict, str]:
        ...
        if search_params.get("trip_type") in ["R", "Y"] and search_params.get(
            "return_date"
        ):
        ...
        response_data, is_success = self._make_request(
            self.endpoints["availability"], payload
        )
    ...
    def get_fare_rules(self, flight_ids: List[str], track_id: str) -> dict:
        ...
        response_data, is_success = self._make_request(
            self.endpoints["fare_rules"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Fare rules request failed"
            )
    ...
    def price_flight(
        self, track_id: str, segment_info: dict, itinerary_info: dict
    ) -> dict:
        """
        ...
        response_data, is_success = self._make_request(
            self.endpoints["pricing"], payload
        )

        if not is_success:
            error_msg = response_data.get("ResponseStatus", {}).get(
    ...
    def get_seat_map(
        self, flight_segments: List[dict], passengers: List[dict], track_id: str
    ) -> dict:
        """
        ...
        response_data, is_success = self._make_request(
            self.endpoints["seat_map"], payload
        )

        if not is_success:
            error_msg = response_data.get("ResponseStatus", {}).get(
                "Error", "Seat map request failed"
            )
    ...
    def create_booking(
        self, booking_data: dict, track_id: str, block_pnr: bool = False
    ) -> dict:
        """
        ...
        if passenger_errors:
            raise AirIQException(
                f"Passenger validation failed: {', '.join(passenger_errors)}"
            )
        ...
        if gst_info.get("number"):
            ...
            if not all(gst_info.get(field) for field in required_gst_fields):
                raise AirIQException(
                    "GST information must be complete (all fields) or not provided at all"
                )
        ...
        payload = {
            ...
            "FFNumberInfo": self._format_frequent_flyer(
                booking_data.get("frequent_flyer", [])
            ),
                ...
                payload["ItineraryFlightsInfo"].append(
                    {
                        ...
                        "PaymentInfo": [
                            {
                                "TotalAmount": str(
                                    it.get("payment_total")
                                    or booking_data.get("total_amount")
                                    or 0
                                )
                            }
                        ],
            ...
            payload["ItineraryFlightsInfo"] = [
                {
                    ...
                    "PaymentMode": "T",
                    "SeatsSSRInfo": self._format_seats_ssr(
                        booking_data.get("seats", [])
                    ),
                    "BaggSSRInfo": self._format_baggage_ssr(
                        booking_data.get("baggage", [])
                    ),
                    "MealsSSRInfo": self._format_meals_ssr(
                        booking_data.get("meals", [])
                    ),
        ...
        response_data, is_success = self._make_request(
            self.endpoints["booking"], payload
        )
    ...
    def _format_seats_ssr(self, seats: List[dict]) -> List[dict]:
        ...
        """
        return [
            {"PaxRefNumber": str(seat["passenger_ref"]), "SeatID": seat["seat_id"]}
            for seat in seats
            if seat.get("seat_id")
        ]
    ...
    def _format_meals_ssr(self, meals: List[dict]) -> List[dict]:
        ...
        """
        return [
            {"MealID": str(meal["meal_id"]), "PaxRefNumber": str(meal["passenger_ref"])}
            for meal in meals
            if meal.get("meal_id")
        ]
    ...
    def issue_ticket(
        ...
        response_data, is_success = self._make_request(
            self.endpoints["ticketing"], payload
        )
    ...
    def get_booking_details(self, airiq_pnr: str) -> dict:
        ...
        payload = {
            ...
            "Item": [{"AirIqPNR": airiq_pnr}],
        }

        response_data, is_success = self._make_request(
            self.endpoints["get_booking"], payload
        )
    ...
    def cancel_booking(
        self, airiq_pnr: str, flag: str = "CANCEL", remarks: str = ""
    ) -> dict:
        """
        ...
        payload = {
            ...
            "OnlineInfo": {"Flag": flag, "AirIqPNR": airiq_pnr, "Remarks": remarks},
        }

        response_data, is_success = self._make_request(
            self.endpoints["cancel"], payload
        )
    ...
    def get_account_balance(self) -> dict:
        ...
        response_data, is_success = self._make_request(
            self.endpoints["balance"], payload
        )
    ...
    def get_agent_balance(self) -> dict:
        ...
        try:
            ...
            if result_code != "1" or error:
                logger.error(
                    f"AirIQ balance check failed - ResultCode: {result_code}, Error: {error}"
                )
                return {
                    "success": False,
                    "message": error
                    or f"Balance check failed with ResultCode: {result_code}",
                    "balance": 0,
            ...
            return {"success": False, "message": str(e), "balance": 0}
        except Exception as e:
    ...
    def get_pricing(self, pricing_data: dict) -> dict:
        ...
        if not track_id or not itinerary_flights:
            raise AirIQException(
                "TrackId and ItineraryFlightsInfo are required for pricing"
            )
    ...
    def track_booking_status(self, booking_track_id: str) -> dict:
        ...
        payload = {
            ...
            "Item": [{"BookingTrackId": booking_track_id}],
        }

        response_data, is_success = self._make_request(
            self.endpoints["track_status"], payload
        )
    ...
    def reschedule_availability(
        self, trip_type: str, airiq_pnr: str, flight_segments: list, remarks: str = ""
    ) -> dict:
        """
        ...
        response_data, is_success = self._make_request(
            self.endpoints["reschedule_avail"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Reschedule availability request failed"
            )
    ...
    def reschedule_booking(
        ...
        response_data, is_success = self._make_request(
            self.endpoints["reschedule"], payload
        )
    ...
    def get_ssr_services(self, airiq_pnr: str, airline_pnr: str) -> dict:
        ...
        response_data, is_success = self._make_request(
            self.endpoints["get_ssr"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "SSR services request failed"
            )
            raise AirIQException(f"SSR services request failed: {error_msg}")

        return response_data

    def add_ssr_services(
        self,
        airiq_pnr: str,
        airline_pnr: str,
        track_id: str,
        meals_ssr: List[dict] = None,
        baggage_ssr: List[dict] = None,
        seats_ssr: List[dict] = None,
        other_ssr: List[dict] = None,
        ...
        payload = {
            ...
            "Payment": [{"PaymentMode": "T", "Amount": str(payment_amount)}],
        }

        response_data, is_success = self._make_request(
            self.endpoints["add_ssr"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Add SSR request failed"
            )
    ...
    def hold_cancel(self, airiq_pnr: str, airline_pnr: str) -> dict:
        ...
        response_data, is_success = self._make_request(
            self.endpoints["hold_cancel"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Hold cancel request failed"
            )
    ...
    def get_multi_class(
        ...
        response_data, is_success = self._make_request(
            self.endpoints["multi_class"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Multi class request failed"
            )
            raise AirIQException(f"Multi class failed: {error_msg}")

        return response_data

    def get_multi_class_fare(
        self,
        flight_ids: List[str],
        class_fare: List[dict],
        adults: int = 1,
        children: int = 0,
        infants: int = 0,
        trip_type: str = "O",
        ...
        if not flight_ids:
            raise AirIQException(
                "At least one FlightID is required for multi-class fare request"
            )
        if not class_fare:
            raise AirIQException("ClassFare is required (one item per FlightID)")
        if len(class_fare) != len(flight_ids):
            logger.warning(
                "ClassFare count does not match FlightIDs count; proceeding but API may fail"
            )
        ...
        for item in class_fare:
            airline_class = (
                item.get("AirlineClass") or item.get("Class") or item.get("class")
            )
            seats_flag = (
                item.get("SeatAvailFlag") or item.get("Seats") or item.get("seats")
            )
            ...
            normalized_class_fare.append(
                {"AirlineClass": str(airline_class), "SeatAvailFlag": str(seats_flag)}
            )
        ...
        response_data, is_success = self._make_request(
            self.endpoints["multi_class_fare"], payload
        )

        if not is_success:
            error_msg = response_data.get("Status", {}).get(
                "Error", "Multi class fare request failed"
            )
    ...
    def validate_passenger_details(self, passengers: List[dict]) -> List[str]:
        ...
        for i, pax in enumerate(passengers, 1):
            ...
            if pax.get("pax_type", "").upper() not in valid_pax_types:
                errors.append(
                    f"{prefix} Invalid passenger type. Must be ADT, CHD, or INF"
                )
