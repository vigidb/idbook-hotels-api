...
class FlightPricingService:
    ...
    def create_pricing_session(
        ...
        try:
            ...
            if track_id and flight_results is not None:
                ...
                departure_date_airiq = datetime.strptime(
                    search_params["departure_date"], "%Y-%m-%d"
                ).strftime("%Y%m%d")
                return_date_airiq = None
                if search_params.get("return_date"):
                    return_date_airiq = datetime.strptime(
                        search_params["return_date"], "%Y-%m-%d"
                    ).strftime("%Y%m%d")
                ...
                search_response, track_id = airiq_service.search_flights(
                    airiq_search_params
                )
            ...
            # Cache the session
            cache.set(
                f"flight_pricing_session:{session_id}",
                pricing_session,
                timeout=self.cache_timeout,
            )

            logger.info(
                f"Created flight pricing session {session_id} with track_id {track_id}"
            )
        ...
        try:
            ...
            return {
                ...
                "time_remaining": max(
                    0,
                    int(
                        (pricing_session["expires_at"] - timezone.now()).total_seconds()
                    ),
                ),
            }
            ...
            logger.error(
                f"Error calculating pricing for session {session_id}: {str(e)}"
            )
    ...
    def calculate_booking_total(
        self, pricing_breakdown: Dict, gst_info: Dict = None
    ) -> Dict:
        """
    ...
    def mark_session_as_booked(self, session_id: str, booking_reference: str) -> bool:
        ...
        try:
            ...
            cache.set(
                f"flight_pricing_session:{session_id}", pricing_session, timeout=86400
            )  # 24 hours

            logger.info(
                f"Marked session {session_id} as booked with reference {booking_reference}"
            )
            return True
                ...
                for item in items:
                    ...
                    if flight_details and fares:
                        option = {
                            ...
                            "destination": flight_details[-1].get(
                                "Destination"
                            ),  # Last segment destination
                            "departure_time": flight_details[0].get(
                                "DepartureDateTime"
                            ),
                            ...
                            "fares": fares,
                            "base_amount": fares[0]
                            .get("Faredescription", [{}])[0]
                            .get("BaseAmount", "0"),
                            "gross_amount": fares[0]
                            .get("Faredescription", [{}])[0]
                            .get("GrossAmount", "0"),
                            "airline_category": flight_details[0].get(
                                "AirlineCategory", "LCC"
                            ),
                            "refundable": flight_details[0].get("Refundable", "False")
                            == "True",
    ...
    def _get_airiq_pricing(
        self, track_id: str, selected_flights: List[Dict], search_params: Dict
    ) -> Dict:
        """Get detailed pricing from AirIQ"""
        try:
            # Validate inputs
            ...
            for flight in selected_flights:
                ...
                base_amount = flight.get(
                    "base_amount",
                    flight.get("BaseAmount", flight.get("baseAmount", "0")),
                )
                gross_amount = flight.get(
                    "gross_amount",
                    flight.get("GrossAmount", flight.get("grossAmount", "0")),
                )
            ...
            logger.info(
                f"Requesting AirIQ pricing for track_id: {track_id}, segments: {len(flight_details['segments'])}, base: {total_base}, gross: {total_gross}"
            )
            ...
            if pricing_response:
                logger.info(
                    f"AirIQ pricing response keys: {list(pricing_response.keys())}"
                )
            else:
    ...
    def _calculate_comprehensive_pricing(
        self, pricing_response: Dict, search_params: Dict, ancillary_services: Dict
    ) -> Dict:
        """Calculate comprehensive pricing breakdown"""
        try:
            # Log the pricing response structure for debugging
            logger.debug(
                f"Pricing response structure: {list(pricing_response.keys()) if pricing_response else 'None'}"
            )

            # Validate pricing response
            if not pricing_response or not isinstance(pricing_response, dict):
                logger.warning(
                    "Pricing response is None or invalid, using fallback pricing"
                )
            ...
            if not price_itinerary_info:
                logger.warning(
                    "No price itinerary info found in response, using fallback"
                )
                return self._create_fallback_pricing(search_params, ancillary_services)
            ...
            ar_list = (
                price_itinerary_info.get("AvailabilityResponse")
                or price_itinerary_info.get("Availability")
                or []
            )
                    ...
                    for fd in fdesc:
                        ...
                        count = (
                            ...
                            if paxtype == "ADT"
                                ...
                                children
                                if paxtype == "CHD"
                                else infants if paxtype == "INF" else 0
                            )
                        )
                        b = Decimal(str(fd.get("BaseAmount", "0")))
                        t = Decimal(
                            str(fd.get("TotalTaxAmount", fd.get("TaxAmount", "0")))
                        )
                        g = Decimal(
                            str(fd.get("GrossAmount", fd.get("TotalAmount", "0")))
                        )
                        if count > 0:
                            base_fare += b * count
                            taxes += t * count
            ...
            if gross_amount == Decimal("0") and fare_breakdown:
                ...
                taxes = Decimal(
                    str(
                        fare_breakdown.get(
                            "TotalTaxAmount", fare_breakdown.get("TaxAmount", "0")
                        )
                    )
                )
                gross_amount = Decimal(
                    str(
                        fare_breakdown.get(
                            "GrossAmount", fare_breakdown.get("TotalAmount", "0")
                        )
                    )
                )
            ...
            seats_total = self._calculate_ancillary_total(
                ancillary_services.get("seats", [])
            )
            meals_total = self._calculate_ancillary_total(
                ancillary_services.get("meals", [])
            )
            baggage_total = self._calculate_ancillary_total(
                ancillary_services.get("baggage", [])
            )
            other_total = self._calculate_ancillary_total(
                ancillary_services.get("other", [])
            )
            ...
            if not passenger_breakdown or (
                passenger_breakdown.get("adults", {}).get("fare_per_person") == 0
                and passenger_breakdown.get("children", {}).get("fare_per_person") == 0
                and passenger_breakdown.get("infants", {}).get("fare_per_person") == 0
            ):
                ...
                if isinstance(ar_list, list) and ar_list:
                    fdesc = (ar_list[0].get("Fares") or [{}])[0].get(
                        "Faredescription"
                    ) or []
                    for fd in fdesc:
                        paxtype = (fd.get("Paxtype") or "ADT").upper()
            ...
            logger.info(
                f"Calculated pricing breakdown: base={base_fare}, taxes={taxes}, gross={gross_amount}, ancillary={ancillary_total}"
            )
            return result
    ...
    def _create_fallback_pricing(
        self, search_params: Dict, ancillary_services: Dict
    ) -> Dict:
        ...
        base_fare += (
            base_fare_per_adult * Decimal("0.75") * children
        )  # 75% for children
        ...
        passenger_breakdown = {
            "adults": {
                ...
                "fare_per_person": float(
                    base_fare_per_adult + (taxes / max(adults, 1))
                ),
            ...
            "infants": {
                ...
                "total": float(
                    base_fare_per_adult * Decimal("0.1") * infants
                    + (taxes * Decimal("0.05"))
                ),
            },
        }
    ...
    def _calculate_passenger_breakdown(
        self, fare_breakdown: Dict, adults: int, children: int, infants: int
    ) -> Dict:
    ...
    def _calculate_gst(self, amount: Decimal, gst_info: Dict = None) -> Dict:
        ...
        try:
            ...
            if is_business:
                ...
                cgst = sgst = (amount * gst_rate / 2).quantize(
