...
class EnhancedFlightSearchViewSet(viewsets.ViewSet, StandardResponseMixin, LoggingMixin):
    """
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'origin': openapi.Schema(type=openapi.TYPE_STRING, description='3-letter IATA origin code'),
                'destination': openapi.Schema(type=openapi.TYPE_STRING, description='3-letter IATA destination code'),
                'departure_date': openapi.Schema(type=openapi.TYPE_STRING, description='Departure date (YYYY-MM-DD)'),
                'return_date': openapi.Schema(type=openapi.TYPE_STRING, description='Return date for round trip (YYYY-MM-DD)'),
                'trip_type': openapi.Schema(type=openapi.TYPE_STRING, enum=['O', 'R'], default='O'),
                'adults': openapi.Schema(type=openapi.TYPE_INTEGER, minimum=1, maximum=9, default=1),
                'children': openapi.Schema(type=openapi.TYPE_INTEGER, minimum=0, maximum=8, default=0),
                'infants': openapi.Schema(type=openapi.TYPE_INTEGER, minimum=0, maximum=4, default=0),
                'flight_class': openapi.Schema(type=openapi.TYPE_STRING, enum=['E', 'B', 'F'], default='E'),
                    ...
                    properties={
                        ...
                        'data': openapi.Schema(
                            ...
                            properties={
                                ...
                                'flight_options': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Items(type=openapi.TYPE_OBJECT)),
                                'expires_at': openapi.Schema(type=openapi.TYPE_STRING),
                                'time_remaining': openapi.Schema(type=openapi.TYPE_INTEGER)
                            }
                        )
                    }
                )
    ...
    def flight_availability(self, request):
        ...
        try:
            ...
            try:
                departure_date = datetime.strptime(request.data['departure_date'], '%Y-%m-%d').date()
            ...
            self.log_info(f"AirIQ Response Keys: {list(flight_results.keys()) if isinstance(flight_results, dict) else 'Not a dict'}")
            ...
            if isinstance(flight_results, dict) and 'Status' in flight_results:
                ...
                self.log_info(f"AirIQ Status - Code: {status_info.get('ResultCode')}, Error: {status_info.get('Error')}")
            ...
            if isinstance(flight_results, dict):
                ...
                if itinerary_list is not None:
                    itinerary_count = len(itinerary_list) if isinstance(itinerary_list, list) else 0
                    self.log_info(f"Found {itinerary_count} itineraries in AirIQ response")
                    if itinerary_count > 0:
                        # Log first itinerary structure
                        first_itinerary = itinerary_list[0]
                        self.log_info(f"First itinerary keys: {list(first_itinerary.keys()) if isinstance(first_itinerary, dict) else 'Not a dict'}")
                else:
                    self.log_warning("ItineraryFlightList is None - no flights returned by AirIQ")
    ...
    def _group_flights_by_number(self, flight_results):
        ...
        # Debug logging
        logger.info(f"Processing {len(itinerary_list) if itinerary_list else 0} itineraries from AirIQ")
            ...
            for item in items:
                ...
                if flight_key not in flight_groups:
                    ...
                    flight_groups[flight_key] = {
                        ...
                        'airline_code': first_flight.get('AirlineDescription', '')[:2] or first_flight.get('OperatingCarrier', ''),
                        ...
                        'fare_type_description': first_flight.get('FareTypeDescription', ''),
                        'fare_description': first_flight.get('FareDescription', ''),
                        'fare_rule_info': first_flight.get('FareRuleInfo', ''),
                        'refundable': first_flight.get('Refundable', 'False').lower() == 'true',
                        ...
                        'aircraft_type': first_flight.get('SegmentDetails', '').split('\r\n')[0] if first_flight.get('SegmentDetails') else ''
                    }
                ...
                for fare in fares:
                    ...
                    if fare_info:
                        ...
                        if (not flight_groups[flight_key]['cheapest_fare'] or 
                            fare_info['gross_amount'] < flight_groups[flight_key]['cheapest_fare']['gross_amount']):
                        ...
                        if (fare_info.get('is_refundable') and 
                            (not flight_groups[flight_key]['refundable_fare'] or 
                             fare_info['gross_amount'] < flight_groups[flight_key]['refundable_fare']['gross_amount'])):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'search_params': openapi.Schema(type=openapi.TYPE_OBJECT, description='Original search parameters from availability'),
                'selected_flights': openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_OBJECT),
                    description='Selected flight with fare option from availability results'
                ),
                'session_id': openapi.Schema(type=openapi.TYPE_STRING, description='Optional existing session ID'),
                'ancillary_services': openapi.Schema(
                    ...
                    properties={
                        ...
                        'meals': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Items(type=openapi.TYPE_OBJECT)),
                        'baggage': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Items(type=openapi.TYPE_OBJECT)),
                        'other': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Items(type=openapi.TYPE_OBJECT))
                    }
                )
            }
        )
    ...
    def get_detailed_pricing(self, request):
        ...
        try:
            ...
            self.log_info(
                ...
                extra={'session_id': session_id, 'user_id': request.user.id if request.user.is_authenticated else None}
            )
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('session_id', openapi.IN_QUERY, type=openapi.TYPE_STRING, required=True),
        ]
    )
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'session_id': openapi.Schema(type=openapi.TYPE_STRING),
                'minutes': openapi.Schema(type=openapi.TYPE_INTEGER, default=5, minimum=1, maximum=15)
            }
        )
    )
