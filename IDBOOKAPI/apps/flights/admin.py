...
class AirIQAPILogAdmin(admin.ModelAdmin):
    ...
    list_filter = ["api_endpoint", "http_method", "result_code", "created_at"]
    search_fields = ["api_endpoint", "request_data", "response_data", "error_message"]
    readonly_fields = ["created_at"]
    date_hierarchy = "created_at"

    fieldsets = (
        ("Request Info", {"fields": ("api_endpoint", "http_method", "request_data")}),
        (
            "Response Info",
...
class FlightSearchSessionAdmin(admin.ModelAdmin):
    ...
    search_fields = ["session_id", "origin", "destination", "airiq_track_id"]
...
class FlightOptionAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("search_session", "inventory_flight")
        )
