...
class Airline(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, help_text="Internal flag to soft-disable airlines in our APIs"
    )
...
class Airport(models.Model):
    ...
    iata_code = models.CharField(
        max_length=3, unique=True, help_text="3-letter IATA airport code"
    )
    icao_code = models.CharField(
        max_length=4, blank=True, help_text="4-letter ICAO airport code"
    )
    name = models.CharField(max_length=200)
...
class FlightRoute(models.Model):
    ...
    origin = models.ForeignKey(
        Airport, on_delete=models.CASCADE, related_name="routes_as_origin"
    )
    destination = models.ForeignKey(
        Airport, on_delete=models.CASCADE, related_name="routes_as_destination"
    )
    airline = models.ForeignKey(
        Airline, on_delete=models.CASCADE, related_name="routes"
    )
    flight_number = models.CharField(
        max_length=10, help_text="Flight number without airline code"
    )
    ...
    base_price = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Base price in INR"
    )
    available_classes = models.JSONField(
...
class FlightInventory(models.Model):
    ...
    route = models.ForeignKey(
        FlightRoute, on_delete=models.CASCADE, related_name="inventory"
    )
    flight_date = models.DateField(help_text="Specific date for this flight")
    departure_datetime = models.DateTimeField(
        help_text="Actual departure date and time"
    )
...
class FlightSearchSession(models.Model):
    ...
    search_mode = models.CharField(
        max_length=10, choices=FLIGHT_BOOKING_MODE, default="REALTIME"
    )
...
class FlightOption(models.Model):
    ...
    search_session = models.ForeignKey(
        FlightSearchSession, on_delete=models.CASCADE, related_name="flight_options"
    )

    # Flight identification
    airiq_flight_id = models.CharField(
        max_length=100, blank=True, help_text="AirIQ Flight ID"
    )
    inventory_flight = models.ForeignKey(
        FlightInventory, on_delete=models.CASCADE, null=True, blank=True
    )
...
class AirIQTokenCache(models.Model):
    ...
    def cache_token(cls, token, expires_in_hours=24, expires_at=None):
        ...
        return cls.objects.create(
            token=token, expires_at=token_expires_at, is_active=True
        )
