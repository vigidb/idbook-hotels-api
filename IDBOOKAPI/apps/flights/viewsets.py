...
from .serializers import (
    ...
    FlightOptionSerializer,
    FlightSearchResultSerializer,
    FlightPricingSerializer,
)
...
class FlightSearchViewSet(viewsets.ViewSet, StandardResponseMixin, LoggingMixin):
    ...
    def search_flights(self, request):
        ...
        try:
            ...
            sorted_options = self._sort_flight_options(
                all_flight_options, search_data.get("sort_by", "price")
            )
    ...
    def _search_inventory(self, search_data):
        ...
        try:
            ...
            if isinstance(search_data["departure_date"], str):
                search_data["departure_date"] = datetime.strptime(
                    search_data["departure_date"], "%Y-%m-%d"
                ).date()
    ...
    def _search_realtime(self, search_data):
        ...
        try:
            ...
            if search_data.get("return_date") and search_data.get("trip_type") in [
                "R",
                "Y",
            ]:
            ...
            realtime_options = self._parse_airiq_results(
                response_data, track_id, search_data
            )
    ...
    def _parse_airiq_results(self, response_data, track_id, search_data):
        ...
        if not response_data.get("ItineraryFlightList"):
            logger.warning(
                f"No ItineraryFlightList found in response: {response_data.keys()}"
            )
        ...
        for flight_group_idx, flight_group in enumerate(
            response_data["ItineraryFlightList"]
        ):
            ...
            logger.info(
                f"Flight group {flight_group_idx + 1} has {len(flight_group['Items'])} items"
            )

            for item_idx, flight_item in enumerate(flight_group["Items"]):
                logger.info(
                    f"Processing flight item {item_idx + 1} in group {flight_group_idx + 1}"
                )
                ...
                if fares:
                    ...
                    if fare_desc:
                        ...
                        logger.info(
                            f"First fare description keys: {list(first_fare_desc.keys())}"
                        )
                        ...
                        logger.info(
                            f"Parsed fare - Base: {base_fare}, Taxes: {taxes}, Total: {total_fare}"
                        )
                else:
                    logger.warning("No fare information found")

                # Extract airline code correctly - it should be airline code, not description
                airline_code = first_segment.get(
                    "AirlineCode", first_segment.get("AirlineDescription", "")
                )[:3]
                ...
                try:
                    flight_option = FlightOption.objects.create(
                        ...
                        departure_datetime=self._parse_airiq_datetime(
                            first_segment.get("DepartureDateTime", "")
                        ),
                        arrival_datetime=self._parse_airiq_datetime(
                            first_segment.get("ArrivalDateTime", "")
                        ),
                        ...
                        is_refundable=first_segment.get("Refundable", "False").lower()
                        == "true",
                    ...
                    logger.info(
                        f"Created FlightOption {flight_option.id}: {flight_option.airline_code} {flight_option.flight_number}"
                    )
    ...
    def _parse_airiq_datetime(self, datetime_str):
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse datetime '{datetime_str}' with format '%d %b %Y %H:%M': {e}"
            )
            ...
            for fmt in alternative_formats:
                try:
                    ...
                    # Make timezone aware using Django's default timezone
                    parsed_dt = tz_utils.make_aware(
                        parsed_dt, tz_utils.get_default_timezone()
                    )
                    logger.info(
                        f"Successfully parsed datetime '{datetime_str}' with alternative format '{fmt}'"
                    )
                    return parsed_dt
                except ValueError:
                    continue
            ...
            logger.error(
                f"Could not parse datetime '{datetime_str}' with any known format"
            )
            return tz_utils.now()
    ...
    def list_airlines(self, request):
        ...
        airline_data = [
            {"code": airline.code, "name": airline.name, "category": airline.category}
...
class FlightPricingViewSet(viewsets.ViewSet, StandardResponseMixin, LoggingMixin):
    ...
    def price_flight(self, request):
        ...
        try:
            ...
            if flight_option.inventory_flight:
                # Inventory pricing
                pricing_data = self._get_inventory_pricing(
                    flight_option, serializer.validated_data
                )
            else:
                # Real-time AirIQ pricing
                pricing_data = self._get_realtime_pricing(
                    flight_option, serializer.validated_data
                )
    ...
    def _get_inventory_pricing(self, flight_option, pricing_data):
        ...
        infant_fare = (
            base_fare_per_person * infants * 0.1
        )  # Infants get 10% of adult fare
        ...
        return {
            ...
            "passenger_breakdown": [
                ...
                },
                {
                    ...
                    "total_per_person": (base_fare_per_person * 0.1)
                    + (taxes_per_person * 0.1),
                },
            ],
    ...
    def _get_realtime_pricing(self, flight_option, pricing_data):
        ...
        try:
                ...
                "segments": [
                    {
                        ...
                        "Origin": flight_option.origin,
                        "Destination": flight_option.destination,
                        "DepartureDateTime": flight_option.departure_datetime.strftime(
                            "%d %b %Y %H:%M"
                        ),
                        "ArrivalDateTime": flight_option.arrival_datetime.strftime(
                            "%d %b %Y %H:%M"
                        ),
                    }
                ],
            ...
            pricing_response = airiq_service.price_flight(
                flight_details, flight_option.search_session.airiq_track_id
            )
