...
from .models import (
    Airline, Airport, FlightRoute, FlightInventory, 
    FlightSearchSession, FlightOption, FlightBooking
)
...
class FlightAPITest(APITestCase):
    ...
    def test_flight_search_invalid_data(self):
        ...
        search_data = {
            'origin': 'DEL',
            'destination': 'BOM',
            'departure_date': (date.today() - timedelta(days=1)).strftime('%Y-%m-%d'),  # Past date
            'adults': 1
        }
