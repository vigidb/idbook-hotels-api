...
class Customer(models.Model):
    ...
    address = models.TextField(
        null=True, blank=True, help_text="Full address", verbose_name="customer_address"
    )
    ...
    date_of_birth = models.DateField(
        null=True, blank=True, help_text="Enter the date of birth of the customer."
    )
    profile_picture = models.FileField(
        upload_to="customer/profile/", blank=True, null=True
    )
    id_proof_type = models.CharField(
        max_length=20,
        choices=KYC_DOCUMENT_CHOICES,
        blank=True,
        null=True,
    )
    ...
    loyalty_points = models.PositiveIntegerField(
        default=0, help_text="Total loyalty points earned by the customer."
    )
    membership_status = models.BooleanField(
        default=False, help_text="Check if the customer has a membership."
    )
...
class WalletTransaction(models.Model):
    ...
    transaction_type = models.CharField(
        max_length=10, choices=TXN_TYPE_CHOICES, help_text="Credit / Debit"
    )
    ...
    payment_medium = models.CharField(
        max_length=50, choices=PAYMENT_MEDIUM, default="Idbook"
    )
    is_transaction_success = models.BooleanField(default=False)
    expiry_date = models.DateTimeField(
        null=True, blank=True, help_text="Expiry date for pro wallet credit amount"
    )
    ...
    status = models.CharField(
        max_length=100, choices=WALLET_STATUS_CHOICES, default="Completed"
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
