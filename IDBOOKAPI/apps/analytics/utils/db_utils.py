...
def create_or_update_property_count(property_id, user_id=None, ip_address=None):
    ...
    try:
        ...
        property_obj = PropertyAnalytics.objects.filter(
            user=user_id, property_info=property_id, created__date=cdate
        )
        if property_obj.exists():
            property_obj.update(visit_count=F("visit_count") + 1)
        else:
            PropertyAnalytics.objects.create(
                user_id=user_id, property_info_id=property_id, visit_count=1
            )
...
def get_property_visit(property_id, date=None, start_date=None, end_date=None):
    prop_analytics_obj = PropertyAnalytics.objects.filter(property_info=property_id)
