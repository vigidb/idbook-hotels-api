...
def property_checkin_count(property_id, date=None, start_date=None, end_date=None):
    ...
    property_checkin_analytics = {
        "date_checkin_count": date_checkin_count,
...
def get_property_revenue(property_id, date=None, start_date=None, end_date=None):
    booking_revenue_obj = BookingPaymentDetail.objects.filter(
    ...
    if date:
        ...
        date_revenue_total = booking_revenue_date_obj.aggregate(Sum("amount")).get(
            "amount__sum", 0
        )
    ...
    if start_date and end_date:
        booking_revenue_drange_obj = booking_revenue_obj.filter(
            created__date__lte=end_date, created__date__gte=start_date
        )
        drange_revenue_total = booking_revenue_drange_obj.aggregate(Sum("amount")).get(
            "amount__sum", 0
        )
    ...
    property_revenue_analytics = {
        "date_revenue_total": date_revenue_total,
...
def get_booking_stats(property_id, filter_condition, start_date=None, end_date=None):
    ...
    meta_status_counts = {
        "booking_created": meta_bookings.filter(
            booking_created_date__isnull=False
        ).count(),
        "booking_confirmed": meta_bookings.filter(
            booking_confirmed_date__isnull=False
        ).count(),
        "booking_cancelled": meta_bookings.filter(
            booking_cancelled_date__isnull=False
        ).count(),
        "booking_completed": meta_bookings.filter(
            booking_completed_date__isnull=False
        ).count(),
    }
    ...
    room_stats = {
        ...
        "booked_rooms": bookings.aggregate(
            total_rooms=Sum("hotel_booking__requested_room_no")
        )["total_rooms"]
        or 0,
    ...
    total_revenue = (
        revenue_data["date_revenue_total"]
        if start_date == end_date
        else revenue_data["drange_revenue_total"]
    )
