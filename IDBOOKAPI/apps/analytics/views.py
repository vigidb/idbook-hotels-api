...
from apps.analytics.utils.analytics_utils import (
    property_checkin_count,
...
class PropertyAnalyticsViewSet(
    ...
    def property_analytics_dashboard(self, request):
        ...
        property_visit_analytics = get_property_visit(
            property_id, date=date, start_date=start_date, end_date=end_date
        )
        # property check in count analytics
        property_checkin_analytics = property_checkin_count(
            property_id, date=date, start_date=start_date, end_date=end_date
        )
        # property revenue total
        property_revenue_analytics = get_property_revenue(
            property_id, date=date, start_date=start_date, end_date=end_date
        )

        analytics = {
            "property_visit": property_visit_analytics,
            "property_checkin": property_checkin_analytics,
        ...
        response = self.get_response(
            data=analytics,
            status="success",
            message="Retrieve Property Analytics Success",
            count=1,
    ...
    def get_property_stats(self, request):
        ...
        stats = {
            "date_range_stats": get_booking_stats(
                property_id, date_range_filter, start_date_obj, end_date_obj
            ),
            "today_stats": get_booking_stats(property_id, today_filter, today, today),
            "week_stats": get_booking_stats(
                property_id, week_filter, week_start, today
            ),
            "month_stats": get_booking_stats(
                property_id, month_filter, month_start, today
            ),
        }
        ...
        for current_date in paginated_dates:
            ...
            daily_stats.append(
                {"day": day_name, "checkIn": checkins, "checkOut": checkouts}
            )
        ...
        while week_start <= end_date_obj:
            ...
            weekly_stats.append(
                {"day": week_name, "checkIn": week_checkins, "checkOut": week_checkouts}
            )
