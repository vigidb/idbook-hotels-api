...
class DynamicRoomPricing(models.Model):
    for_property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="property_dynamic_pricing"
    )
    for_room = models.ForeignKey(
        Room, on_delete=models.CASCADE, related_name="room_dynamic_pricing"
    )
...
class TopDestinations(models.Model):
    ...
    no_of_hotels = models.PositiveIntegerField(
...
class PropertyCommission(models.Model):
    ...
    property_comm = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="property_commission"
    )
