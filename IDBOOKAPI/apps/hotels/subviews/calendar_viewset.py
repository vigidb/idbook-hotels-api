...
class PropertyCalendarViewSet(
    ...
    def list(self, request, *args, **kwargs):
        ...
        data = {
            "room_booked_details": serializer.data,
...
class RoomPricingCalendarViewset(
    ...
    def validate_create_parameters(self):
        ...
        if not error_list:
            is_price_set = check_is_room_dynamic_price_set(
                for_room, start_date, end_date
            )
            if is_price_set:
                error_list.append(
                    {
                        "field": "unknown",
                        "message": "Pricing overlaps for the given date range",
        ...
        if not error_list:
            dynamic_pricing_dict = {
                "for_property_id": for_property,
                "for_room_id": for_room,
    ...
    def validate_update_parameters(self, instance):
        ...
        if not error_list:
            is_price_set = check_is_room_dynamic_price_set(
                instance.for_room, start_date, end_date, instance_id=instance.id
            )
            if is_price_set:
                error_list.append(
                    {
    ...
    def make_active_or_inactive(self, request, pk):
        ...
        if active:
            ...
            if is_price_set:
                message = (
                    "Pricing overlaps for the given date range.You can't make it active"
                )
                response = self.get_error_response(
                    message=message,
                    status="error",
