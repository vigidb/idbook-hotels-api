...
class PropertyPayoutViewset(viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        payload, property_payout_list, batch_id = get_payout_property_details(
            property_ids
        )
        payout_response = initiate_payout(
            payload, property_payout_list, payment_medium, batch_id
        )
        ...
        if payout_status == "partial_success" or payout_status == "success":
            ...
            if payout_success_list:
                payout_success_obj = PropertyPayoutDetails.objects.filter(
                    id__in=payout_success_list
                )
                payout_success_serializer = PropertyPayoutDetailsSerializer(
                    payout_success_obj, many=True
                )
                data["payout_success"] = payout_success_serializer.data

            if payout_error_list:
                payout_error_obj = PropertyPayoutDetails.objects.filter(
                    id__in=payout_error_list
                )
                payout_error_serializer = PropertyPayoutDetailsSerializer(
                    payout_error_obj, many=True
                )
    ...
    def get_payout_pending_booking_details(self, request):
        ...
        prop_booking_obj, booking_ids, final_payout_amount = get_booking_payout_details(
            property_id
        )
        ...
        data = {
            "total_payout_amount": final_payout_amount,
            "payout_details": booking_payout_serialzer.data,
        }
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="set-batch-payment",
        url_name="set-batch-payment",
        permission_classes=[IsAuthenticated],
    )
    def create_payout_batch(self, request):
        ...
        if not booking_ids:
            custom_response = self.get_error_response(
                message="No booking associated with hotel payout",
                status="error",
                errors=[],
