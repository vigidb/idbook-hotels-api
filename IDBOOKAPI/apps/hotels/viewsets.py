...
from .serializers import (
    ...
    RuleSerializer,
    InclusionSerializer,
    FinancialDetailSerializer,
    HotelAmenityCategorySerializer,
    RoomAmenityCategorySerializer,
    PropertyGallerySerializer,
    RoomGallerySerializer,
    PropertyListSerializer,
    PropertyRetrieveSerializer,
    PropertyBankDetailsSerializer,
    MonthlyPayAtHotelEligibilitySerializer,
)
from .serializers import (
    BlockedPropertySerializer,
    RoomNameSerializer,
    PayAtHotelSpendLimitSerializer,
)
...
from .tasks import (
    send_hotel_sms_task,
    send_hotel_email_task,
    create_service_agreement_task,
)
...
class PropertyViewSet(
    ...
    def property_filter_ops(self):
        ...
        for key in param_dict:
            ...
            if key == "location":
                ...
                query = reduce(
                    ...
                    (
                        ...
                        | Q(title__icontains=param_value.strip())
                        for loc in loc_list
                    ),
                )
            ...
            if key == "property_search":
                query = Q(name__icontains=param_value.strip()) | Q(
                    title__icontains=param_value.strip()
                )
                self.queryset = self.queryset.filter(query)
        ...
        if start_price is not None and end_price is not None:
            property_list = hotel_db_utils.get_property_from_price_range(
    ...
    def checkin_checkout_based_filter(self):
        ...
        if checkin_date and checkout_date:
            ...
            nonavailable_property_list, available_property_dict = (
                hotel_utils.get_filled_property_list(checkin_date, checkout_date)
            )
    ...
    def calculate_complete_pricing_details(
        self, request, queryset, available_property_dict, nonavailable_property_list
    ):
        ...
        for property_obj in queryset:
            ...
            try:
                ...
                self.no_of_days = max(
                    (self.checkout_datetime - self.checkin_datetime).days, 1
                )
                ...
                if not is_allocated:
                    error_message = getattr(
                        ...
                        "message",
                        "Could not find enough rooms to accommodate all guests",
                    )
                    error_code = getattr(
                        allocation_response.data, "error_code", "INADEQUATE_ROOMS"
                    )

                    complete_pricing_details[property_id] = {
                        "status": "error",
                        "error": {"message": error_message, "error_code": error_code},
                    }
                    continue
                ...
                if not is_status:
                    error_message = getattr(
                        allocation_response.data, "message", "Room allocation failed"
                    )
                    error_code = getattr(
                        allocation_response.data, "error_code", "ALLOCATION_ERROR"
                    )
                ...
                if not is_cal_status:
                    error_message = getattr(
                        calculation_response.data, "message", "Price calculation failed"
                    )
                    error_code = getattr(
                        calculation_response.data, "error_code", "CALCULATION_ERROR"
                    )
                ...
                total_final_amount = sum(
                    room_detail.get("final_room_total", 0)
                    for room_detail in self.confirmed_room_details
                )
                ...
                if user and user.is_authenticated:
                    ...
                    if pro_member_discount_value > 0:
                        total_final_amount = (
                            total_final_amount - pro_member_discount_value
                        )
                ...
                total_discount = float(total_room_discount) + float(
                ...
                print(
                    f"Error calculating complete pricing for property {property_id}: {str(e)}"
                )
                complete_pricing_details[property_id] = {
                    "status": "error",
                    "error": {
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if (
                request.data.get("pay_at_hotel") == True
                or request.data.get("pay_at_hotel") == "true"
            ):
            ...
            if prop_status == "Active":
                ...
                send_hotel_sms_task.apply_async(
                    kwargs={
                        "notification_type": "HOTEL_PROPERTY_DEACTIVATION",
                        "params": {"property_id": instance.id},
                    }
                )
                ...
                send_hotel_sms_task.apply_async(
                    kwargs={
                        "notification_type": "HOTEL_PROPERTY_SUBMISSION",
                        "params": {"property_id": instance.id},
                    }
                )
    ...
    def list(self, request, *args, **kwargs):
        ...
        available_property_dict, nonavailable_property_list = (
            self.checkin_checkout_based_filter()
        )
        ...
        self.queryset = hotel_db_utils.get_lowest_property_pricing(
            self.queryset, self.checkin_date, booking_slot
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        instance = hotel_db_utils.get_lowest_property_pricing(
            instance, start_date, booking_slot
        )
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="verify-agreement",
        url_name="verify_agreement",
    ...
    def verify_agreement(self, request):
        ...
        if not token:
            return Response(
                {"success": False, "message": "Invalid request. Token is required."},
                status=400,
            )
        ...
        try:
            for media_data in media.values():
                ...
                property_gallery = PropertyGallery(
                    media=media_data, property_id=property_id
                )
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="media/inactive",
        url_name="media-inactive",
        permission_classes=[IsAuthenticated],
    )
    def make_media_inactive(self, request):
        ...
        update_status = PropertyGallery.objects.filter(id__in=image_ids).update(
            active=active
        )
        ...
        try:
            ...
            if favorite_list:
                ...
                response = self.get_response(
                    ...
                    count=1,
                    status="success",
                    message="Property added to favorite list",
                    status_code=status.HTTP_200_OK,
                )
                return response
        except Exception as e:
    ...
    @action(
        detail=False,
        methods=["GET"],
        url_path="location/autosuggest",
        url_name="location-autosuggest",
        permission_classes=[AllowAny],
    )
    def get_location_suggestion(self, request):
        ...
        location_queryset = self.queryset.filter(
            Q(city_name__icontains=location)
            | Q(state__icontains=location)
            | Q(country__icontains=location)
        ).exclude(state="", country="")
        if location_queryset.count():
            obj_list = (
                location_queryset.values("city_name", "state", "country")
                .distinct("city_name", "state", "country")
                .order_by("city_name")
            )
            ...
            if not user_query:
                response = self.get_response(
                    data=autosuggest_dict,
                    count=1,
                    status="success",
                    message="Location Autosuggest",
                    status_code=status.HTTP_200_OK,
                )
                return response
        ...
        if user_query:
            ...
            user_suggest_dict = {
                "location": autosuggest_dict,
                "property_info": property_info_dict,
            }

            response = self.get_response(
                data=user_suggest_dict,
                count=1,
                status="success",
                message="Location or Property Autosuggest",
                status_code=status.HTTP_200_OK,
            )
            return response
    ...
    def get_property_location(self, request):
        ...
        obj_list = (
            self.queryset.filter(status="Active")
    ...
    def property_insights(self, request, pk=None):
        try:
            ...
            total_customers = (
                HotelBooking.objects.filter(confirmed_property=property_obj)
            ...
            total_transactions = (
                ...
                ).aggregate(total_amount=Sum("final_amount"))["total_amount"]
                or 0
            )
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="set/spend-limit",
        url_name="set-spend-limit",
        permission_classes=[IsAuthenticated],
    )
    def set_pay_at_hotel_spend_limit(self, request):
        ...
        if serializer.is_valid():
            ...
            response_data = PayAtHotelSpendLimitSerializer(
    ...
    @action(
        detail=False,
        methods=["PATCH"],
        url_path="update/spend-limit",
        url_name="update-spend-limit",
        permission_classes=[IsAuthenticated],
    )
    def update_pay_at_hotel_spend_limit(self, request):
        ...
        try:
            ...
            if id is not None:
                ...
                spend_limit = PayAtHotelSpendLimit.objects.get(
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="delete/spend-limit",
        url_name="delete-spend-limit",
        permission_classes=[IsAuthenticated],
    )
    ...
    @action(
        ...
        methods=["GET"],
        permission_classes=[IsAuthenticated],
        url_path="hotel-customer-eligibility",
        url_name="hotel-customer-eligibility",
    )
    def get_customer_eligibility(self, request):
        ...
        serialized_data = MonthlyPayAtHotelEligibilitySerializer(
            queryset, many=True
        ).data
...
class RoomViewSet(
    viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin, ValidationMixin
):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if property_id:
                ...
                hotel_db_utils.room_based_property_update(
                    property_id, starting_price_details, is_slot_price_enabled
                )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            response = super().update(
                request._request.__class__(data=data), *args, **kwargs
            )
            # response = super().update(request, *args, **kwargs)
    ...
    def make_room_inactive(self, request, pk):
        ...
        if property_id:
            starting_price_details = hotel_db_utils.get_slot_based_starting_room_price(
                property_id
            )
    ...
    def make_media_inactive(self, request):
        ...
        # make media active or inactive
        update_status = RoomGallery.objects.filter(id__in=image_ids).update(
            active=active
        )
...
class BlockedPropertyViewSet(
    ...
    def validate_create_parameters(self):
        ...
        if not no_of_blocked_rooms:
            error_list.append(
                {
                    "field": "no_of_blocked_rooms",
                    "message": "Missing room count",
        ...
        if not error_list:
            block_overlap = hotel_db_utils.check_room_blocked(
                blocked_room, start_date, end_date
            )
            if block_overlap:
                error_list.append(
                    {
                        "field": "unknown",
                        "message": "Already blocked withing the date range.",
        ...
        if not error_list:
            ...
            if room_rejected_list:
                error_list.append(
                    {
                        "field": "unknown",
                        "message": "Room not available for the date range",
        ...
        if not error_list:
            ...
            blocked_property_dict = {
                "blocked_property_id": blocked_property,
                "blocked_room_id": blocked_room,
                "no_of_blocked_rooms": no_of_blocked_rooms,
    ...
    def validate_update_parameters(self, instance):
        ...
        block_overlap = hotel_db_utils.check_room_blocked(
            blocked_room, start_date, end_date, instance_id=instance.id
        )
        if block_overlap:
            error_list.append(
                {
    ...
    def retrieve_available_room(self, request):
        ...
        room_list = hotel_utils.get_available_room(
            start_date, end_date, blocked_property
        )
    ...
    def make_active_or_inactive(self, request, pk):
        ...
        response = self.get_response(
            ...
            count=1,
            status="success",
            message="Property Update Success",
            status_code=status.HTTP_200_OK,
        )
        return response
...
class HotelAmenityCategoryViewSet(
    viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin
):
...
class RoomAmenityCategoryViewSet(
    viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin
):
...
class PropertyBankDetailViewSet(
    viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin
):
