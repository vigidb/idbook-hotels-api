...
def generate_existing_contact_report():
    ...
    """
    properties = Property.objects.filter(status="Active").exclude(
        Q(email="") | Q(email__isnull=True) | Q(phone_no="") | Q(phone_no__isnull=True)
    )
...
def create_user_without_otp(
    email, name, mobile_number, password="Idbook@123", group_name="HOTELIER-GRP"
):
    """
    ...
    try:
        ...
        if user and group_name:
            ...
            if email_grp_users and (
                group_name != "B2C-GRP" or db_utils.is_role_exist(email_grp_users, role)
            ):
        ...
        if mobile_number:
            ...
            if mobile_grp_users and (
                group_name != "B2C-GRP"
                or db_utils.is_role_exist(mobile_grp_users, role)
            ):
                print(f"Mobile number {mobile_number} already exists for this group.")

        formatted_name = (
            f"Hotelier {name[:10]}..." if len(name) > 10 else f"Hotelier {name}"
        )
        ...
        if user:
            ...
            user = User(email=email, mobile_number=mobile_number, name=formatted_name)
            user.set_password(password)
            user.save()
        ...
        try:
            if prop.email not in users_by_email:
                user = create_user_without_otp(
                    email=prop.email, name=prop.name, mobile_number=prop.phone_no
                )
...
def send_activation_notifications_to_properties():
    ...
    for prop in properties:
        ...
        if not prop.managed_by_id:
            print(
                f"✗ Skipping property {prop.name} (ID: {prop.id}) - No manager assigned."
            )
            continue
        ...
        if not user:
            print(
                f"✗ Skipping property {prop.name} (ID: {prop.id}) - Managed user not found."
            )
            continue
        ...
        if prop.phone_no:
            try:
                ...
                print(
                    f"✓ Queued activation SMS for property: {prop.name} (ID: {prop.id})"
                )
                sms_success_count += 1
            except Exception as e:
                print(
                    f"✗ Failed to queue SMS for property: {prop.name} (ID: {prop.id}). Error: {e}"
                )
        ...
        if prop.email:
            try:
                ...
                print(
                    f"✓ Queued activation email for property: {prop.name} (ID: {prop.id})"
                )
                email_success_count += 1
            except Exception as e:
                print(
                    f"✗ Failed to queue email for property: {prop.name} (ID: {prop.id}). Error: {e}"
                )
