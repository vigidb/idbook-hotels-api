    ...
    for room in rooms:
        ...
        if room.room_occupancy:
            ...
            for field in ["base_adults", "max_adults", "max_children", "max_occupancy"]:
                if field in room.room_occupancy and is_numeric_string(
                    room.room_occupancy[field]
                ):
        ...
        if room.room_price:
            ...
            fields = [
                ...
                "base_rate",
                "extra_bed_price",
                "extra_bed_price_4hrs",
                "extra_bed_price_8hrs",
                "extra_bed_price_12hrs",
            ]
            for field in fields:
                if field in room.room_price and is_numeric_string(
                    room.room_price[field]
                ):
                    price_issues[field] = room.room_price[field]

            if "child_bed_price" in room.room_price and isinstance(
                room.room_price["child_bed_price"], list
            ):
                child_issues = []
                for i, child in enumerate(room.room_price["child_bed_price"]):
                    ...
                    if "age_limit" in child and isinstance(child["age_limit"], list):
                        age_issues = [
                            {"index": j, "value": v}
                            for j, v in enumerate(child["age_limit"])
                    ...
                    for f in [
                        "child_bed_price",
                        "child_bed_price_4hrs",
                        "child_bed_price_8hrs",
                        "child_bed_price_12hrs",
                    ]:
...
def scan_dynamic_pricing_models():
    ...
    for pricing in all_prices:
        ...
        if pricing.room_price:
            ...
            for field in fields:
                if field in pricing.room_price and is_numeric_string(
                    pricing.room_price[field]
                ):
                    price_issues[field] = pricing.room_price[field]

            if "child_bed_price" in pricing.room_price and isinstance(
                pricing.room_price["child_bed_price"], list
            ):
                child_issues = []
            ...
            prices_with_issues.append(
                {
                    ...
                    "room_name": room_name,
                    "start_date": (
                        pricing.start_date.strftime("%Y-%m-%d %H:%M:%S")
                        if pricing.start_date
                        else None
                    ),
                    "end_date": (
                        pricing.end_date.strftime("%Y-%m-%d %H:%M:%S")
                        if pricing.end_date
                        else None
                    ),
                    "issues": issues,
        ...
        if "age_limit" in child and isinstance(child["age_limit"], list):
            for j, age in enumerate(child["age_limit"]):
                if is_numeric_string(age):
                    changes.append(
                        f"child_bed_price[{i}].age_limit[{j}]: '{age}' -> {int(age)}"
                    )
...
def convert_room_data(room_obj):
    ...
    mod_occ, chg_occ = convert_dict_values(
        occupancy_data, ["base_adults", "max_adults", "max_children", "max_occupancy"]
    )
    if mod_occ:
    ...
    if "child_bed_price" in price_data and isinstance(
        price_data["child_bed_price"], list
    ):
...
def update_all_properties_starting_prices():
    ...
    for prop in all_properties:
        property_id = prop.id
        has_active_rooms = Room.objects.filter(
