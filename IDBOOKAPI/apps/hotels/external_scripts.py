...
def update_property_starting_price():
    ...
    for property_id in property_list:
        ...
        update_property_with_starting_price(property_id, starting_price_details)


def update_property_slot_price():
    property_list = get_slot_price_enabled_property()
    print(property_list)
    data = Property.objects.filter(id__in=property_list).update(
...
def check_property_coordinates():
    objs = Property.objects.values(
        "id", "address__coordinates__lat", "address__coordinates__lng"
    ).exclude(address__coordinates__lat="", address__coordinates__lng="")
    for obj in objs:
        try:
...
def create_default_property_spend_limits():
    ...
    for prop in properties:
        ...
        PayAtHotelSpendLimit.objects.create(property=prop, property_spend_limit=50000)
        created_count += 1
