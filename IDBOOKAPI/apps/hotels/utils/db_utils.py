...
def get_property_featured_image(property_id):
    property_gallery = PropertyGallery.objects.filter(
        property_id=property_id, featured_image=True, active=True
    ).first()
    return property_gallery
...
def get_property_room_for_booking(property_id: int, room_id: int):
    room_detail = (
        Room.objects.filter(id=room_id, property_id=property_id)
        .values(
            "id", "room_type", "room_price", "is_extra_bed_available", "room_occupancy"
        )
        .first()
    )
    return room_detail
...
def get_favorite_property(user_id):
    favorite_list = FavoriteList.objects.filter(
        user_id=user_id, property__isnull=False
...
def get_starting_room_price(property_id):
    try:
        starting_price = (
            Room.objects.annotate(val=KT("room_price__base_rate"))
...
def get_slot_based_starting_room_price(property_id):
    try:
        ...
        starting_price_slot_list = (
            room_obj.filter(is_slot_price_enabled=True)
            .annotate(
        ...
        starting_price_list = room_obj.annotate(
            base_price=Cast(KT("room_price__base_rate"), IntegerField())
        ).aggregate(starting_base_price=Coalesce(Min("base_price"), 0))
...
def room_based_property_update(
    property_id, starting_price_details, is_slot_price_enabled
):
    try:
        Property.objects.filter(id=property_id).update(
            starting_price_details=starting_price_details,
            is_slot_price_enabled=is_slot_price_enabled,
        )
    except Exception as e:
        print(e)
...
def get_property_from_price_range(start_price, end_price):
    property_list = Room.objects.filter(
        room_price__base_rate__gte=start_price, room_price__base_rate__lte=end_price
    ).values_list("property_id", flat=True)
...
def get_price_range(location_list=[], slot="24 Hrs"):
    ...
    if slot == "4 Hrs":
        min_price = room_obj.annotate(
            val=Cast(KT("room_price__price_4hrs"), IntegerField())
        ).aggregate(min=Coalesce(Min("val"), 0))
        max_price = room_obj.annotate(
            val=Cast(KT("room_price__price_4hrs"), IntegerField())
        ).aggregate(max=Coalesce(Max("val"), 0))
    elif slot == "8 Hrs":
        min_price = room_obj.annotate(
            val=Cast(KT("room_price__price_8hrs"), IntegerField())
        ).aggregate(min=Coalesce(Min("val"), 0))
        max_price = room_obj.annotate(
            val=Cast(KT("room_price__price_8hrs"), IntegerField())
        ).aggregate(max=Coalesce(Max("val"), 0))
        ...
        min_price = room_obj.annotate(
            val=Cast(KT("room_price__price_12hrs"), IntegerField())
        ).aggregate(min=Coalesce(Min("val"), 0))
        max_price = room_obj.annotate(
            val=Cast(KT("room_price__price_12hrs"), IntegerField())
        ).aggregate(max=Coalesce(Max("val"), 0))
        ...
        # 24 hrs
        min_price = room_obj.annotate(
            val=Cast(KT("room_price__base_rate"), IntegerField())
        ).aggregate(min=Coalesce(Min("val"), 0))
        max_price = room_obj.annotate(
            val=Cast(KT("room_price__base_rate"), IntegerField())
        ).aggregate(max=Coalesce(Max("val"), 0))
...
def get_slot_price_enabled_property():
    property_list = Room.objects.filter(is_slot_price_enabled=True).values_list(
...
def filter_property_by_room_amenity(room_amenity):
    ...
    if room_amenity:
        ...
        property_list = Room.objects.filter(query_room_amenity).values_list(
            "property_id", flat=True
        )
    return list(property_list)
...
def check_is_room_dynamic_price_set(room_id, start_date, end_date, instance_id=None):
    dynamic_pricing_obj = DynamicRoomPricing.objects.filter(
        for_room=room_id, start_date__lt=end_date, end_date__gt=start_date, active=True
    )
    # for update
    if instance_id:
...
def get_blocked_room_list(property_id, start_date, end_date):
    blocked_room_list = BlockedProperty.objects.filter(
        ...
        active=True,
    ).values_list("blocked_room", flat=True)
...
def get_blocked_property(start_date, end_date):
    ...
    blocked_property_room = (
        blocked_property.filter(is_entire_property=False)
        .values("blocked_room")
        .annotate(total_blocked_rooms=Sum("no_of_blocked_rooms"))
    )
...
def get_dynamic_room_pricing_list(start_date, end_date, room_ids: list):
    ...
    pricing_objs = DynamicRoomPricing.objects.filter(for_room__in=room_ids, active=True)
...
def get_dynamic_pricing_with_date_list(room_id, date_list):
    pricing_obj = DynamicRoomPricing.objects.filter(for_room_id=room_id, active=True)
    date_price_dict = {}
    is_price_available = False
    if pricing_obj.exists():
        for date in date_list:
            date_pricing_obj = (
                pricing_obj.filter(start_date__date__lte=date, end_date__date__gte=date)
                .values("room_price")
                .first()
            )
            if date_pricing_obj:
...
def get_property_count_by_location(location_name):
    ...
    property_objs = property_objs.filter(
        Q(city_name=location_name.strip())
...
def update_destination_property_count(location_name, total_count):
    destination_objs = TopDestinations.objects.filter(location_name=location_name)
...
def check_top_destination_exist(location_list):
    existing_list = list(
        TopDestinations.objects.filter(location_name__in=location_list).values_list(
...
def get_lowest_property_pricing(queryset, checkin_date, booking_slot):
    ...
    dynamic_pricing_subquery = (
        dynamic_pricing_subquery.annotate(
            val=Cast(KT(dynamic_room_rate), IntegerField())
        )
        .values("val")
        .order_by("val")[:1]
    )
    ...
    queryset = queryset.annotate(
        dynamic_price=Coalesce(Subquery(dynamic_pricing_subquery), 999999999)
    )

    # get the lowest price by comparing dynamic lowest price
    # and normal starting price
    queryset = queryset.annotate(
        lowest_price=Least("dynamic_price", Cast(KT(normal_rate), IntegerField()))
    )
...
def get_property_distance(queryset, lat, lon):
                ...
                Cast(
                    (
                        ...
                        * Cos(Radians(property_lng) - Radians(lon))
                        + Sin(Radians(lat)) * Sin(Radians(property_lat))
                    ),
                    FloatField(),
                )
            )
    ...
    queryset = queryset.annotate(distance=expression_wrapper)
...
def query_annotate():
    ...
    property_obj = Property.objects.annotate(
        dynamic_price=Coalesce(
            Subquery(
                DynamicRoomPricing.objects.filter(for_property_id=OuterRef("pk"))
                .annotate(val=Cast(KT("room_price__base_rate"), IntegerField()))
                .values("val")
                .order_by("val")[:1]
            ),
            999999999,
        ...
        lowest_price=Least(
            "dynamic_price",
            Cast(KT("starting_price_details__starting_base_price"), IntegerField()),
        )
    )
...
def save_unavailable_property_search(query_params):
    ...
    if search_term:
        UnavailableProperty.objects.create(
            search_term=search_term, full_params=dict(query_params)
        )
...
def get_user_booking_data(user_id, booking_date):
    ...
    monthly_eligibility = MonthlyPayAtHotelEligibility.objects.filter(
        user_id=user_id, month=current_month_name
    ).first()

    spent_amount = (
        monthly_eligibility.spent_amount if monthly_eligibility else Decimal("0.00")
    )
    ...
    if spend_limit_obj:
        if (
            spend_limit_obj.cancel_count
            and total_cancel_count >= spend_limit_obj.cancel_count
        ) or (
            spend_limit_obj.cancel_limit
            and cancel_limit >= spend_limit_obj.cancel_limit
        ):
            is_eligible = False
            is_blacklisted = True
...
def bulk_update_property_payout_details(payout_list, payout_data):
    PropertyPayoutDetails.objects.filter(id__in=payout_list).update(**payout_data)


def update_property_payout_error_details(tnx_id, payout_data):
    prop_payout_obj = PropertyPayoutDetails.objects.filter(
