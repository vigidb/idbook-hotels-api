...
class HotelAmenity(models.Model):
    amenity_category = models.ForeignKey(
        HotelAmenityCategory, on_delete=models.DO_NOTHING, related_name="hotel_amenity"
    )
    title = models.CharField(max_length=200, unique=True)
...
class RoomAmenity(models.Model):
    room_amenity_category = models.ForeignKey(
        RoomAmenityCategory, on_delete=models.DO_NOTHING, related_name="room_amenity"
    )
    title = models.CharField(max_length=200, unique=True)
...
def default_starting_price_json():
    starting_price_json = {
        "starting_4hr_price": 0,
        "starting_8hr_price": 0,
        "starting_12hr_price": 0,
        "starting_base_price": 0,
    }
...
class Property(models.Model):
    ...
    service_category = models.CharField(
        max_length=255,
        choices=SERVICE_CATEGORY_TYPE_CHOICES,
        default="",
    ...
    name = models.CharField(
        max_length=70, db_index=True, help_text="Name of the property."
    )
    ...
    property_type = models.CharField(
        max_length=50, choices=PROPERTY_TYPE, default="Hotel"
    )
    rental_form = models.CharField(
        max_length=50, choices=RENTAL_FORM, default="Private room"
    )

    checkin_time = models.TimeField(
        null=True, help_text="Check-in time for the property."
    )
    checkout_time = models.TimeField(
        null=True, help_text="Check-out time for the property."
    )
    ...
    email_list = models.JSONField(
        null=True, default=dict, help_text='["email 1", "email 2"]'
    )
    ...
    phone_no_list = models.JSONField(
        null=True, default=dict, help_text='["phone 1", "phone 2"]'
    )
    description = models.TextField(blank=True, default="")
    website_list = models.JSONField(
        null=True, default=dict, help_text='["website link 1", "website link 2"]'
    )
    ...
    starting_price_details = models.JSONField(
        null=True, default=default_starting_price_json
    )

    rating = models.DecimalField(
        max_digits=3, decimal_places=2, default=0.0, help_text="Rating of the property."
    )
    ...
    legal_document = models.FileField(
        upload_to="hotels/property/legal-document/", null=True
    )

    current_page = models.PositiveIntegerField(
        default=0, help_text="Pages completed for property in Front End"
    )
    review_star = models.DecimalField(
        max_digits=3,
        decimal_places=2,
        default=0.0,
    ...
    additional_fields = models.JSONField(
        null=True,
        default=default_property_additional_fields_json,
        help_text="Additional Fields related to the property",
    )
    status = models.CharField(
        max_length=50, choices=HOTEL_STATUS, default="In-Progress"
    )
    ...
    property_measurement_type = models.CharField(
        max_length=25, choices=ROOM_MEASUREMENT, default=""
    )
    pay_at_hotel = models.BooleanField(
        default=True, help_text="If true, customer can pay at the hotel."
    )
    created = models.DateTimeField(
        auto_now_add=True, help_text="Date and time when the property was created."
    )
    updated = models.DateTimeField(
        auto_now=True, help_text="Date and time when the property was last updated."
    )
    service_agreement_pdf = models.FileField(
        upload_to="hotels/service_agreements/", blank=True, null=True
    )
...
class Room(models.Model):
    ...
    room_type = models.CharField(
        max_length=25, choices=ROOM_CHOICES, blank=True, default=""
    )
    room_view = models.CharField(
        max_length=25, choices=ROOM_VIEW_CHOICES, blank=True, default=""
    )
    bed_type = models.CharField(
        max_length=25, choices=BED_TYPE_CHOICES, blank=True, default=""
    )
    name = models.CharField(
        max_length=70, blank=True, default="", help_text="Name of the property."
    )
    description = models.TextField(blank=True, null=True)

    room_size = models.PositiveSmallIntegerField(default=0, help_text="Room Size")
    room_measurement_type = models.CharField(
        max_length=25, choices=ROOM_MEASUREMENT, default=""
    )
    no_available_rooms = models.PositiveSmallIntegerField(default=0)
    meal_options = models.CharField(
        max_length=100, choices=MEAL_OPTIONS, blank=True, default=""
    )
    is_smoking_allowed = models.BooleanField(default=False)
    extra_bed_type = models.CharField(
        max_length=30, choices=EXTRA_BED_TYPE, blank=True, default=""
    )
    ...
    discount = models.PositiveSmallIntegerField(
        default=0, help_text="Discount percentage for the room (maximum 90%)."
    )
    ...
    created = models.DateTimeField(
        auto_now_add=True, help_text="Date and time when the room was created."
    )
    updated = models.DateTimeField(
        auto_now=True, help_text="Date and time when the room was last updated."
    )
...
class PropertyGallery(models.Model):
    property = models.ForeignKey(
        Property, on_delete=models.SET_NULL, null=True, related_name="gallery_property"
    )
...
class BlockedProperty(models.Model):
    blocked_property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="blocked_property"
    )
    blocked_room = models.ForeignKey(
        Room, on_delete=models.CASCADE, null=True, related_name="blocked_room"
    )
...
class RoomGallery(models.Model):
    room = models.ForeignKey(
        Room, on_delete=models.SET_NULL, null=True, related_name="gallery_room"
    )
...
class Gallery(models.Model):
    ...
    created = models.DateTimeField(
        auto_now_add=True, help_text="Date and time when the gallery image was created."
    )
    updated = models.DateTimeField(
        auto_now=True,
...
class PropertyBankDetails(models.Model):
    property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="property_bank"
    )
...
class PropertyPayoutDetails(models.Model):
    OWNERSHIP_CHOICES = (("ADMIN", "ADMIN"), ("AUTO", "AUTO"))

    payout_property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="property_payout"
    )
    amount = models.DecimalField(max_digits=20, decimal_places=6)
    transaction_id = models.CharField(max_length=100, blank=True)
    batch_id = models.CharField(max_length=100, blank=True)
    batch_created_by = models.CharField(
        max_length=20, choices=OWNERSHIP_CHOICES, blank=True
    )
    ...
    transaction_executed_by = models.CharField(
        max_length=20, choices=OWNERSHIP_CHOICES, blank=True
    )
    payout_reference_file = models.FileField(
        upload_to="hotels/payout/transaction/", blank=True, null=True
    )
...
class PayAtHotelSpendLimit(models.Model):
    start_limit = models.PositiveIntegerField(
        default=0, help_text="Start range of bookings for paying at hotel"
    )
    ...
    cancel_limit = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0.00,
    ...
    cancel_count = models.PositiveIntegerField(
        default=0, help_text="Number of cancellation for this booking range"
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
    # For pro membership
    ...
    property_spend_limit = models.DecimalField(
...
class MonthlyPayAtHotelEligibility(models.Model):
    is_eligible = models.BooleanField(
        default=False, help_text="Is the user eligible for Pay at Hotel this month?"
    )
    eligible_limit = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0.00,
        null=True,
    ...
    total_booking_count = models.PositiveIntegerField(
        default=0, null=True, help_text="Total bookings made by the user this month"
    )
    cancel_limit = models.DecimalField(
        max_digits=10,
        decimal_places=2,
    ...
    total_cancel_count = models.PositiveIntegerField(
        default=0, null=True, help_text="Allowed cancel count"
    )
    is_blacklisted = models.BooleanField(
        default=False, help_text="Is the user blacklisted this month?"
    )
    updated_by = models.CharField(
        max_length=50, default="Automatic", help_text="Who updated this record?"
    )
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="monthly_pay_at_hotel_eligibility"
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
