    ...
    try:
        ...
        if notification_type == "HOTEL_PROPERTY_ACTIVATION":
            ...
            if property and property.phone_no:
                ...
                send_sms(
                    property.phone_no, "HOTEL_PROPERTY_ACTIVATION", variables_values
                )
            ...
            if property and property.phone_no:
                ...
                send_sms(
                    property.phone_no, "HOTEL_PROPERTY_DEACTIVATION", variables_values
                )
                create_hotelier_notification(
                    property, notification_type, variables_values
                )
            ...
            if property and property.phone_no:
                ...
                send_sms(
                    property.phone_no, "HOTELER_PAYMENT_NOTIFICATION", variables_values
                )
                create_hotelier_notification(
                    property, notification_type, variables_values
                )
            ...
            review = (
                Review.objects.select_related("property")
                .filter(id=params.get("review_id"))
                .first()
            )
            if review and review.property and review.property.phone_no:
                ...
                create_hotelier_notification(
                    review.property, notification_type, variables_values
                )
            ...
            if property and property.phone_no:
                ...
                send_sms(
                    property.phone_no, "HOTEL_PROPERTY_SUBMISSION", variables_values
                )
                create_hotelier_notification(
                    property, notification_type, variables_values
                )
            ...
            if property and property.phone_no:
                ...
                short_link = shorten_link(website_link)
                variables_values = f"Hotelier|{property.name}|{short_link}"
                send_sms(property.phone_no, "HOTELIER_PAH_FEATURE", variables_values)
                create_hotelier_notification(
                    property, notification_type, variables_values
                )
            ...
            if property and property.phone_no and booking:
                variables_values = (
                    f"Hotelier|{property.name}|{float(booking.final_amount)}"
                )
                send_sms(
                    property.phone_no, "HOTELIER_PAH_BOOKING_ALERT", variables_values
                )
                create_hotelier_notification(
                    property, notification_type, variables_values
                )
            ...
            try:
                ...
                if property and property.email:
                    ...
                    email_template = get_template(
                        "email_template/hotelier-active-property.html"
                    )
                    ...
                    print(
                        f"Property not found or no email available for property_id: {property_id}"
                    )
...
def update_monthly_pay_at_hotel_eligibility_task(self, user_id, booking_date):
    ...
    try:
        ...
        existing_eligibility = MonthlyPayAtHotelEligibility.objects.filter(
            user_id=user_id, month=data["month_name"]
        ).first()
        ...
        if existing_eligibility and existing_eligibility.updated_by == "Admin":
            ...
            monthly_eligibility, created = (
                MonthlyPayAtHotelEligibility.objects.update_or_create(
                    user_id=user_id,
                    ...
                    defaults={
                        ...
                        "is_eligible": existing_eligibility.is_eligible
            ...
            data["is_eligible"] = (
                existing_eligibility.is_eligible and remaining_limit > 0
            )
        ...
        print(
            f"Updated monthly pay at hotel eligibility for user {user_id}, month: {data['month_name']}"
        )
        print(
...
@celery_idbook.task(bind=True)
def create_service_agreement_task(
    self, property_id, is_verified=False, verified_at=None, ip_address=None
):
    ...
    try:
        ...
        if property:
            ...
            if is_verified and verified_at and ip_address:
                ...
                pdf_context["verified_date"] = verified_date.strftime(
                    "%d-%B-%Y %H:%M:%S"
                )
            ...
            try:
                print(
                    f"Generating {'verified' if is_verified else 'initial'} service agreement PDF"
                )
                if is_verified:
                    ...
                    pdf_result = generate_service_agreement_pdf(
                        pdf_context, property_id=property_id, save_to_db=False
                    )
                ...
                if property.email:
                    if is_verified:
                        ...
                        send_agreement_email(
                            property, pdf_result, email_context, is_verification=True
                        )
                        print(
                            f"Verification confirmation email sent to {property.email}"
                        )
                    else:
                        ...
                        send_agreement_email(
                            property, pdf_result, email_context, is_verification=False
                        )
...
def send_hotel_receipt_email_task(self, booking_id):
    try:
        ...
        context = {
            ...
            "day_count": (
                room_details.get("no_of_days") if booking_slot == "24 Hrs" else None
            ),
            "hour_count": (
                room_details.get("booking_slot") if booking_slot != "24 Hrs" else None
            ),
            ...
            "discount_text": (
                f"INR {float(booking.discount)}" if booking.discount else "None"
            ),
        ...
        try:
            pdf_bytes = generate_hotel_receipt_pdf(
                context, booking_id=booking_id, booking_obj=booking
            )
