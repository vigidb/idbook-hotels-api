...
router.register(
    r"refund-and-cancellation-policy",
    RefundAndCancellationPolicyViewSet,
    basename="refund_and_cancellation_policy",
)
router.register(
    r"terms-and-conditions", TermsAndConditionsViewSet, basename="terms_and_conditions"
)
...
router.register(
    r"user-subscription", UserSubscriptionViewset, basename="user-subscription"
)
router.register(
    r"feature-subscriptions",
    FeatureSubscriptionViewSet,
...
urlpatterns = [
    path(
        "get-state-and-district/<str:query>/",
        GetDistrictStateView.as_view(),
        name="district-state",
    ),
]
