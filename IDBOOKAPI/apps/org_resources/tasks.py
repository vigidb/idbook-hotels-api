...
from apps.org_resources.utils.subscription_utils import (
    subscription_recurring_notification,
    subscription_recurring_debit,
            ...
            if booking and property and float(booking.final_amount) > 20000:
                ...
                for user in users:
                    if user.mobile_number:
                        ...
                        admin_create_notification(
                            user, notification_type, variables_values
                        )
    ...
    try:
        ...
        if notification_type == "PRO_MEMBER_REFFERAL_BONUS":
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{referral_code}|{bonus_amount}"
                )
                send_sms(
                    user.mobile_number, "PRO_MEMBER_REFFERAL_BONUS", variables_values
                )
                create_pro_member_notification(
                    user, notification_type, variables_values
                )
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{failure_reason}|{renewal_deadline}"
                )
                send_sms(
                    user.mobile_number,
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{renewal_date}|{payment_amount}"
                )
                send_sms(
                    user.mobile_number,
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{renewal_date}|{renewal_amount}"
                )
                send_sms(
                    user.mobile_number,
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{membership_type}|{expiry_date}"
                )
                send_sms(user.mobile_number, "PRO_MEMBER_WELCOME", variables_values)
                create_pro_member_notification(
                    user, notification_type, variables_values
                )
            ...
            if user and user.mobile_number:
                variables_values = (
                    f"{user.name or 'User'}|{discount_amount}|{hotel_name}"
                )
                send_sms(user.mobile_number, "PRO_MEMBER_DISCOUNT", variables_values)
                create_pro_member_notification(
                    user, notification_type, variables_values
                )
