...
class UserSubscriptionSerializer(serializers.ModelSerializer):
    ...
    subscription_type = serializers.CharField(
        source="idb_sub.subscription_type", allow_null=True
    )
    features = serializers.SerializerMethodField()
    ...
    def validate(self, data):
        ...
        if start_limit is not None and end_limit is not None:
            if start_limit > end_limit:
                raise serializers.ValidationError(
                    "start_limit cannot be greater than end_limit"
                )
        ...
        if hasattr(self, "instance") and self.instance:
            if start_limit is not None and end_limit is None:
                if start_limit > self.instance.end_limit:
                    raise serializers.ValidationError(
                        f"start_limit cannot be greater than existing end_limit ({self.instance.end_limit})"
                    )
            ...
            if end_limit is not None and start_limit is None:
                if end_limit < self.instance.start_limit:
                    raise serializers.ValidationError(
                        f"end_limit cannot be less than existing start_limit ({self.instance.start_limit})"
                    )
