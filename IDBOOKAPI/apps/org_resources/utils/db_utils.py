...
def is_corporate_email_exist(email):
    is_exist = CompanyDetail.objects.filter(company_email=email).exists()
    return is_exist


def is_corporate_number_exist(company_phone):
    is_exist = CompanyDetail.objects.filter(company_phone=company_phone).exists()
    return is_exist
...
def fetch_subscription_for_mandate_check(current_date):
    ...
    user_subscriptions = user_subscriptions.filter(
        Q(last_mandate_check__isnull=True) | Q(last_mandate_check__lte=mandate_date)
    )
...
def fetch_rec_subscribers_to_notify(current_date, payment_medium=None):
    try:
        user_subscriptions = UserSubscription.objects.filter(
            notification_id="",
            next_notify_date__date=current_date,
...
def fetch_rec_subscribers_to_debit(current_date, payment_medium=None):
    try:
        user_subscriptions = UserSubscription.objects.filter(
            recrinit_tnx_id="",
            next_payment_date__date=current_date,
            mandate_status="active",
...
def update_subrecur_transaction(transaction_id, trans_dict: dict):
    ...
    trans_obj = SubRecurringTransaction.objects.get(recrinit_tnx_id=transaction_id)
    if trans_dict:
