...
def booking_comfirmed_notification_template(
    booking_id, booking_type, confirmation_code, notification_dict
):
    try:
        # Notification
...
The confirmation code is: {confirmation_code}".format(
            booking_type=booking_type, confirmation_code=confirmation_code
        )
...
def booking_cancelled_notification_template(
    booking_id, booking_type, cancellation_code, notification_dict
):
    try:
        # Notification
...
The cancellation code is: {cancellation_code}".format(
            booking_type=booking_type, cancellation_code=cancellation_code
        )
        redirect_url = "/bookings/{booking_id}".format(booking_id=booking_id)
...
def booking_completed_notification_template(
    booking_id, booking_type, notification_dict
):
    try:
        # Notification
...
is LOW ({wallet_balance} INR)".format(
...
def wallet_booking_balance_notification_template(
    booking, wallet_balance, notification_dict
):
    try:
...
def generate_user_notification(
    ...
    try:
        ...
        if group_name is None:
            group_name = (
                "CORPORATE-GRP"
                if booking and getattr(booking, "company_id", None)
                else "B2C-GRP"
            )
        ...
        if notif_type in [
            ...
            "ELIGIBILITY_LOSS_WARNING",
            "ELIGIBILITY_LOSS_NOTIFICATION",
...
def admin_create_notification(user, notification_type, variables_values):
    try:
        ...
        app_notification_type = (
            "BOOKING"
            if notification_type
            in ["ADMIN_PAH_HIGH_VALUE_ALERT", "ADMIN_PAH_PAYMENT_DISPUTE_ALERT"]
            else "GENERAL"
        )
