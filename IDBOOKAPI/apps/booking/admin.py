...
class FlightPassengerInline(admin.TabularInline):
    ...
    extra = 0
    fields = ('passenger_reference', 'passenger_type', 'title', 'first_name', 'last_name', 
              'date_of_birth', 'gender', 'passport_number', 'infant_with_passenger')
    readonly_fields = ('passenger_reference', 'passenger_type', 'title', 'first_name', 'last_name', 
                      'date_of_birth', 'gender', 'passport_number', 'infant_with_passenger')
    can_delete = False
...
class FlightAncillaryServiceInline(admin.TabularInline):
    ...
    extra = 0
    fields = ('passenger', 'service_type', 'service_code', 'service_description', 'service_price')
    readonly_fields = ('passenger', 'service_type', 'service_code', 'service_description', 'service_price')
    can_delete = False

class FlightBookingAdmin(admin.ModelAdmin):
    list_display = ('id', 'booking_reference', 'airiq_pnr', 'airline_pnr', 'status', 
                    'flying_from', 'flying_to', 'departure_date', 'flight_trip', 'related_booking_link')
    list_filter = ('status', 'flight_trip', 'departure_date')
    search_fields = ('booking_reference', 'airiq_pnr', 'airline_pnr', 'airiq_track_id')
    readonly_fields = ('id', 'booking_reference', 'airiq_pnr', 'airline_pnr', 'airiq_track_id')
    ...
    fieldsets = (
        ...
        ('Hold Management', {
            'fields': ('hold_expires_at', 'payment_expires_at')
        }),
        ('Session Data', {
            'fields': ('search_session_data', 'selected_flight_data', 'pricing_validation_data'),
            'classes': ('collapse',)
        }),
        ('Fare Rules', {
            'fields': ('fare_rules',),
    ...
    def related_booking_link(self, obj):
        ...
        try:
            ...
            return format_html('<a href="{}">{}</a>', url, booking.confirmation_code or f"Booking #{booking.id}")
        except Booking.DoesNotExist:
            return '-'
    related_booking_link.short_description = 'Related Booking'

class FlightPassengerAdmin(admin.ModelAdmin):
    list_display = ('id', 'passenger_reference', 'passenger_type', 'full_name', 'date_of_birth', 
                    'gender', 'flight_booking_link', 'booking_link')
    list_filter = ('passenger_type', 'gender', 'created_at')
    search_fields = ('first_name', 'last_name', 'passport_number', 'flight_booking__booking_reference')
    readonly_fields = ('id', 'created_at', 'updated_at')
    
    fieldsets = (
        ('Passenger Information', {
            'fields': ('id', 'passenger_reference', 'passenger_type', 'title', 'first_name', 'last_name', 
                      'date_of_birth', 'gender')
        }),
        ('Travel Documents', {
            'fields': ('passport_number', 'passport_expiry', 'passport_issued_date', 'passport_country_code')
        }),
        ('Infant & Frequent Flyer', {
            'fields': ('infant_with_passenger', 'frequent_flyer_number', 'frequent_flyer_airline')
        }),
        ('Ticket Information', {
            'fields': ('ticket_number', 'seat_number')
        }),
        ('Relationships', {
            'fields': ('flight_booking', 'booking')
        }),
        ('Timestamps', {
    ...
    def flight_booking_link(self, obj):
        if obj.flight_booking:
            url = reverse('admin:booking_flightbooking_change', args=[obj.flight_booking.id])
            return format_html('<a href="{}">{}</a>', url, obj.flight_booking.booking_reference)
        return '-'
    ...
    def booking_link(self, obj):
        if obj.booking:
            ...
            return format_html('<a href="{}">{}</a>', url, obj.booking.confirmation_code)
        return '-'
    booking_link.short_description = 'Main Booking'

class FlightAncillaryServiceAdmin(admin.ModelAdmin):
    list_display = ('id', 'service_type', 'service_code', 'service_description', 'service_price', 
                    'passenger_link', 'flight_booking_link')
    list_filter = ('service_type', 'created_at')
    search_fields = ('service_code', 'service_description', 'passenger__first_name', 'passenger__last_name')
    readonly_fields = ('id', 'created_at', 'updated_at')
    
    fieldsets = (
        ('Service Information', {
            'fields': ('id', 'service_type', 'airiq_service_id', 'service_code', 'service_description')
        }),
        ('Pricing', {
            'fields': ('service_price',)
        }),
        ('Segment Details', {
            'fields': ('segment_reference',)
        }),
        ('Relationships', {
            'fields': ('flight_booking', 'passenger')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
    ...
    def passenger_link(self, obj):
        if obj.passenger:
            return f"{obj.passenger.full_name} (Ref: {obj.passenger.passenger_reference})"
        return '-'
...
class BookingAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'booking_type', 'confirmation_code', 'invoice_id', 'status',
                    'created', 'updated', 'flight_booking_link')
