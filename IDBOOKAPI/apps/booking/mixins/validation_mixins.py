from IDBOOKAPI.utils import get_days_from_string, get_date_from_string
from apps.booking.utils.db_utils import get_booking_based_tax_rule
...
class ValidationMixins:
    ...
    def validate_pre_confirm_booking(self):
        ...
        if not isinstance(child_age_list, list):
            error_info = {
                "message": "invalid child age list format",
        ...
        if child_count != len(child_age_list):
            error_info = {
                "message": "Mismatch between child count and age list",
                "error_code": "MISMATCH_CHILD_COUNT_AGE",
            }
            return False, error_info

        self.confirmed_checkin_time = self.request.data.get(
            "confirmed_checkin_time", None
        )
        self.confirmed_checkout_time = self.request.data.get(
            "confirmed_checkout_time", None
        )

        if not self.confirmed_checkin_time or not self.confirmed_checkout_time:
            error_info = {
                "message": "check in and check out missing",
        ...
        self.no_of_days = get_days_from_string(
            self.confirmed_checkin_time,
            self.confirmed_checkout_time,
            string_format="%Y-%m-%dT%H:%M%z",
        )

        if self.no_of_days is None:
            error_info = {
                "message": "Error in date conversion",
