    ...
    def payment_property_filter_ops(self):
        ...
        for key in param_dict:
            ...
            if key == "property":
                ...
                filter_dict["hotel_booking__confirmed_checkin_time__gte"] = (
                    start_checking_date
                )
            elif key == "end_checkin_date":
                end_checkin_date = param_value.replace(" ", "+")
                filter_dict["hotel_booking__confirmed_checkin_time__lte"] = (
                    end_checkin_date
                )
            elif key == "is_transaction_success":
                is_transaction_success = True if param_value == "true" else False
                filter_dict["booking_payment__is_transaction_success"] = (
