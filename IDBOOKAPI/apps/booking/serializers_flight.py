...
class GSTInfoSerializer(serializers.Serializer):
    ...
    mobile = serializers.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^\+?1?\d{9,15}$", message="Invalid mobile number format"
            )
        ],
    )
...
class PassengerDetailsSerializer(serializers.Serializer):
    ...
    date_of_birth = serializers.DateField(
        required=False, allow_null=True, input_formats=["%d/%m/%Y", "%Y-%m-%d"]
    )
    ...
    passport_country_code = serializers.CharField(
...
class AncillaryServiceSerializer(serializers.Serializer):
    ...
    service_type = serializers.ChoiceField(
        choices=[("BAGGAGE", "Baggage"), ("MEAL", "Meal"), ("OTHER", "Other")]
    )
...
class FlightBookingRequestSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if adults != adult_count:
            raise serializers.ValidationError(
                f"Adult count mismatch: expected {adult_count}, got {adults}"
            )
        if children != child_count:
            raise serializers.ValidationError(
                f"Child count mismatch: expected {child_count}, got {children}"
            )
        if infants != infant_count:
            raise serializers.ValidationError(
                f"Infant count mismatch: expected {infant_count}, got {infants}"
            )
...
class BookingSearchSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(choices=FLIGHT_BOOKING_STATUS, required=False)
