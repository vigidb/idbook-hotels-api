...
class FlightBookingStatusTracker:
    ...
    def __init__(self, booking: Booking = None, booking_id: int = None):
        if booking:
            ...
            self.booking = (
                Booking.objects.filter(id=booking_id, booking_type="FLIGHT")
                .select_related("flight_booking", "user")
                .first()
            )
        else:
    ...
    def get_current_status(self) -> Dict:
        ...
        if self.booking.status == "confirmed":
            status_info["confirmed_at"] = (
                self.flight_booking.confirmed_at.isoformat()
                if self.flight_booking.confirmed_at
                else None
            )
        elif self.booking.status == "canceled":
            status_info["cancelled_at"] = (
                self.flight_booking.cancelled_at.isoformat()
                if self.flight_booking.cancelled_at
                else None
            )
        elif self.flight_booking.status == "HELD":
            status_info["hold_expires_at"] = (
                self.flight_booking.hold_expires_at.isoformat()
                if self.flight_booking.hold_expires_at
                else None
            )
    ...
    def update_status_from_airiq(self) -> Dict:
        ...
        try:
            ...
            airiq_status = self.airiq.get_booking_status(
    ...
    def check_flight_schedule_updates(self) -> Dict:
        ...
        try:
            ...
            flight_details = self.airiq.get_flight_details(
            ...
            schedule_changes = self._check_schedule_changes(
                flight_details.get("data", {})
            )
    ...
    def get_booking_timeline(self) -> List[Dict]:
        ...
        # Payment events
        payments = BookingPaymentDetail.objects.filter(booking=self.booking).order_by(
            "created"
        )
            ...
            timeline.append(
                {
                    ...
                    "timestamp": (
                        self.flight_booking.cancelled_at.isoformat()
                        if self.flight_booking.cancelled_at
                        else self.booking.updated.isoformat()
                    ),
                    "status": "canceled",
        ...
        if (
            self.flight_booking.departure_date
            and self.flight_booking.departure_date > timezone.now()
        ):
            timeline.append(
                {
    ...
    def _get_flight_details(self) -> Dict:
        ...
        return {
            ...
            "departure_date": (
                self.flight_booking.departure_date.isoformat()
                if self.flight_booking.departure_date
                else None
            ),
            "arrival_date": (
                self.flight_booking.arrival_date.isoformat()
                if self.flight_booking.arrival_date
                else None
            ),
            "return_date": (
                self.flight_booking.return_date.isoformat()
                if self.flight_booking.return_date
                else None
            ),
    ...
    def _get_payment_status(self) -> Dict:
        ...
        latest_payment = (
            BookingPaymentDetail.objects.filter(booking=self.booking)
            .order_by("-created")
            .first()
        )
        ...
        if not latest_payment:
            return {
                ...
                "pending_amount": float(
                    self.booking.final_amount - self.booking.total_payment_made
                ),
            }
    ...
    def _get_ticket_status(self) -> Dict:
        ...
        return {
            ...
            "ticket_url": (
                self.flight_booking.flight_ticket.url
                if self.flight_booking.flight_ticket
                else None
            ),
    ...
    def _update_from_airiq_data(self, airiq_data: Dict):
        ...
        try:
            ...
            if (
                "AirlinePNR" in airiq_data
                and airiq_data["AirlinePNR"] != self.flight_booking.airline_pnr
            ):
            ...
            if "FlightDetails" in airiq_data:
                ...
                if (
                    "FlightNumber" in flight_details
                    and flight_details["FlightNumber"] != self.flight_booking.flight_no
                ):
            ...
            if "FlightDetails" in airiq_flight_data:
                ...
                if "DepartureTime" in flight_details:
                    ...
                    if (
                        ...
                        and abs(
                            (
                                new_departure - self.flight_booking.departure_date
                            ).total_seconds()
                        )
                        > 300
                    ):  # 5 minutes threshold
                ...
                if "ArrivalTime" in flight_details:
                    new_arrival = datetime.fromisoformat(flight_details["ArrivalTime"])
                    if (
                        self.flight_booking.arrival_date
                        and abs(
                            (
                                new_arrival - self.flight_booking.arrival_date
                            ).total_seconds()
                        )
                        > 300
                    ):
    ...
    def _notify_schedule_changes(self, changes: List[Dict]):
        ...
        try:
            ...
            logger.info(
                f"Schedule changes detected for booking {self.booking.id}: {len(changes)} changes"
            )
...
class FlightBookingRetriever:
    ...
    def get_booking_details(booking_id: int, user=None) -> Dict:
        ...
        try:
            ...
            booking = (
                Booking.objects.select_related("flight_booking", "user", "meta_info")
                .prefetch_related(
                    "flight_booking__flight_passengers",
                    "flight_booking__flight_ancillary_services",
                    "booking_payment",
                )
                .filter(id=booking_id, booking_type="FLIGHT")
                .first()
            )
            ...
            booking_details = {
                ...
                "flight_details": FlightBookingRetriever._get_flight_details(
                    booking.flight_booking
                ),
                "passenger_details": FlightBookingRetriever._get_passenger_details(
                    booking.flight_booking
                ),
                ...
                "ancillary_services": FlightBookingRetriever._get_ancillary_services(
                    booking.flight_booking
                ),
                "status_info": current_status,
                "timeline": status_tracker.get_booking_timeline(),
                "user_details": (
                    FlightBookingRetriever._get_user_details(booking.user)
                    if booking.user
                    else None
                ),
            }
            ...
            if booking.invoice_id:
                booking_details["invoice_details"] = (
                    FlightBookingRetriever._get_invoice_details(booking)
                )
            ...
            return {"success": False, "error": str(e), "error_code": "RETRIEVAL_ERROR"}
    ...
    def get_user_flight_bookings(user, filters: Dict = None) -> Dict:
        ...
        try:
            # Base query
            queryset = (
                Booking.objects.select_related("flight_booking", "user")
                .filter(user=user, booking_type="FLIGHT")
                .order_by("-created")
            )
            ...
            if filters:
                ...
                if "origin" in filters:
                    queryset = queryset.filter(
                        flight_booking__flying_from__icontains=filters["origin"]
                    )
                if "destination" in filters:
                    queryset = queryset.filter(
                        flight_booking__flying_to__icontains=filters["destination"]
                    )
                ...
                bookings.append(
                    {
                        ...
                        "departure_date": (
                            booking.flight_booking.departure_date.isoformat()
                            if booking.flight_booking.departure_date
                            else None
                        ),
    ...
    def _get_flight_details(flight_booking: FlightBooking) -> Dict:
        ...
        return {
            ...
            "arrival_date": (
                flight_booking.arrival_date.isoformat()
                if flight_booking.arrival_date
                else None
            ),
            "return_date": (
                flight_booking.return_date.isoformat()
                if flight_booking.return_date
                else None
            ),
            "return_arrival_date": (
                flight_booking.return_arrival_date.isoformat()
                if flight_booking.return_arrival_date
                else None
            ),
            ...
            "flight_ticket_url": (
                flight_booking.flight_ticket.url
                if flight_booking.flight_ticket
                else None
            ),
        }
            ...
            passengers.append(
                {
                    ...
                    "passport_expiry": (
                        passenger.passport_expiry.isoformat()
    ...
    def _get_pricing_details(booking: Booking) -> Dict:
        ...
        return {
            ...
            "gst_percentage": (
                float(booking.gst_percentage) if booking.gst_percentage else 0
            ),
            ...
            "pro_member_discount": (
                float(booking.pro_member_discount_value)
                if booking.pro_member_discount_value
                else 0
            ),
            ...
            services.append(
                {
                    ...
                    "service_price": (
                        float(service.service_price) if service.service_price else 0
                    ),
                    "passenger_name": (
                        f"{service.passenger.first_name} {service.passenger.last_name}"
        ...
        try:
            ...
            if invoice:
                return {
                    ...
                    "invoice_pdf_url": (
                        invoice.invoice_pdf.url if invoice.invoice_pdf else None
                    ),
                }
        except Exception as e:
