...
def generate_business_code(business_detail):
    ...
    if not location_code and business_detail.country:
        ...
        location_code = (
            clean_country[:3] if len(clean_country) >= 3 else clean_country[:2]
        )
...
def create_invoice_number(billed_by_id=None, gstin=None):
    ...
    """
    print(
        f"DEBUG create_invoice_number called with billed_by_id={billed_by_id}, gstin={gstin}"
    )
    ...
    if billed_by_id:
        qs = Invoice.objects.filter(
            billed_by_id=billed_by_id, invoice_number__startswith=prefix
        )
    else:
...
def invoice_json_hotel_booking(hotel_booking):
    ...
    for confirmed_room in confirmed_room_details:
        ...
        room_amount_with_discount = confirmed_room.get(
            "room_amount_with_discount", None
        )
...
No of Days:: {no_of_days} "
        ...
        item = {
            ...
            "room_discount": room_discount_value,
            "final_room_total": final_room_total,
            "room_amount_with_discount": room_amount_with_discount,
        }
        items.append(item)
...
def invoice_json_flight_booking(booking):
    ...
    def _format_segment(segment):
        ...
        if segment.get("MealsPreference"):
            extras.append(
                f"Meal: {segment.get('MealsPreference')} ({segment.get('MealsAmount', '0')})"
            )
        if segment.get("BaggagePreference"):
            extras.append(
                f"Baggage: {segment.get('BaggagePreference')} ({segment.get('BaggageAmount', '0')})"
            )
        if segment.get("SeatPreference"):
            extras.append(
                f"Seat: {segment.get('SeatPreference')} ({segment.get('SeatAmount', '0')})"
            )
    ...
    if itinerary_details:
            ...
            for leg in itinerary.get("Item", []):
                ...
                for traveller in traveller_info:
                    ...
                    passengers_info.append(
                        f"{pax_name} ({pax_type}) - Ticket {ticket_number}"
                    )

                    segment_information = traveller.get("SegmentInformation", {}).get(
                        "Item", []
                    )
                ...
                item = {
                    ...
                    "price": (
                        float(line_total)
                        if line_total
                        else float(_to_decimal(itinerary.get("TotalAmount")))
                    ),
                    "amount": (
                        float(line_total)
                        if line_total
        ...
        total_discount = float(booking.discount or 0) + float(
            booking.pro_member_discount_value or 0
        )
        ...
        departure_time = (
            departure_date.strftime("%d-%m-%Y %H:%M") if departure_date else "TBD"
        )
        arrival_time = (
            arrival_date.strftime("%d-%m-%Y %H:%M") if arrival_date else "TBD"
        )
        ...
        if flight_trip == "ONE-WAY":
            ...
            return_dep_time = (
                return_date.strftime("%d-%m-%Y %H:%M") if return_date else "TBD"
            )
            return_arr_time = (
                return_arrival_date.strftime("%d-%m-%Y %H:%M")
                if return_arrival_date
                else "TBD"
            )
            ...
            description = (
                f"Flight Class: {flight_class}, Route: {flying_from} → {flying_to}"
            )
        ...
        main_item = {
            ...
            "final_total": max(
                float(flight_booking.base_fare or 0) - float(booking.discount or 0), 0
            ),
        }
...
def invoice_json_data(
    ...
    billed_to = {"name": "", "address": "", "GSTIN": "", "PAN": ""}
    supply_details = {"countryOfSupply": "", "placeOfSupply": ""}
    item = {"name": "", "description": "", "quantity": 0, "price": 0, "amount": 0}
    total, gst = 0, 0
    subtotal = 0
    discount = 0
    notes = ""
    ...
    if bus_details:
        ...
        billed_by = {
            ...
            "address": bus_details.full_address,
            "GSTIN": bus_details.gstin_no,
            "PAN": bus_details.pan_no,
            "email": bus_details.business_email,
            "website": bus_details.website_url,
            "hsn_sac_no": bus_details.hsn_sac_no,
        }
    ...
    if company_details:
        billed_to = {
            ...
            "address": company_details.registered_address,
            "GSTIN": company_details.gstin_no,
            "PAN": company_details.pan_no,
        }
        supply_details = {
            "countryOfSupply": company_details.country,
        ...
        pan = (
            customer_details.pan_card_number if customer_details.pan_card_number else ""
        )

        billed_to = {
            "name": customer_details.user.name,
            "address": address,
            "GSTIN": "",
            "PAN": customer_details.pan_card_number,
        }
        supply_details = {"countryOfSupply": country, "placeOfSupply": state}
    ...
    if booking:
        ...
        if booking_type == "HOTEL":
            ...
            if booking.flight_booking:
                ...
                gst_type = (
                    booking.gst_type
                    if booking.gst_type
                    else ("CGST/SGST" if is_same_state else "IGST")
                )
                subtotal = float(booking.subtotal)
        ...
        if (
            booking.pro_member_discount_value
            and float(booking.pro_member_discount_value) > 0
        ):
        ...
        payload = json.dumps(
            {
                ...
                "dueDate": invoice_due_date.isoformat(),
                "notes": notes,
                "billedBy": billed_by,
                "billedTo": billed_to,
...
def create_invoice(payload):
    url = "{invoice_url}/api/invoices".format(invoice_url=invoice_url)
    ...
    try:
        ...
        response = {
            ...
            "data": {
                ...
                "totalAmount": float(
                    invoice.total_amount or (invoice.total + invoice.total_tax)
                ),
                "status": invoice.status or "Pending",
                "nextScheduleDate": (
                    invoice.next_schedule_date.isoformat()
                    if invoice.next_schedule_date
                    else None
                ),
...
def calculate_flight_gst(booking, business_state=None):
    ...
    if not base_amount or base_amount <= 0:
        return {"gst_percentage": 0, "gst_amount": 0, "gst_type": "NO_GST"}
    ...
    if (
        customer_state
        and business_state_lower
        and customer_state == business_state_lower
    ):
        gst_type = "CGST/SGST"
    else:
        gst_type = "IGST"
...
def generate_invoice_pdf(payload, booking_id=None):
    ...
    try:
        ...
        invoice_data["invoiceDate"] = invoice_data.get(
            "invoiceDate"
        ) or invoice_data.get("invoice_date")
        invoice_data["dueDate"] = invoice_data.get("dueDate") or invoice_data.get(
            "due_date"
        )
        invoice_data["billedBy"] = dict(
            invoice_data.get("billedBy") or invoice_data.get("billed_by_details") or {}
        )
        invoice_data["billedTo"] = dict(
            invoice_data.get("billedTo") or invoice_data.get("billed_to_details") or {}
        )
        invoice_data["supplyDetails"] = dict(
            invoice_data.get("supplyDetails")
            or invoice_data.get("supply_details")
            or {}
        )
        invoice_data["GSTType"] = invoice_data.get("GSTType") or invoice_data.get(
            "GST_type"
        )
        ...
        for key in ["invoiceDate", "dueDate"]:
            if invoice_data.get(key):
                try:
                    invoice_data[key] = datetime.fromisoformat(
                        str(invoice_data[key]).replace("Z", "+00:00")
                    )
                except Exception:
                    pass
        ...
        try:
            html_content = render_to_string(
                "invoice_template/invoice.html", invoice_data
            )
        except Exception:
            html_content = render_to_string(
                "invoice_template/manual_invoice.html", invoice_data
            )
