...
def process_ssr_success(
    ...
    try:
        ...
        try:
            retr = (
                airiq_resp.get("Retrieveresponse")
                or airiq_resp.get("Retriveresponse")
                or {}
            )
        ...
        for key, items in selections:
            ...
            for it in items or []:
                pax_ref = (
                    it.get("PaxRefId") or it.get("PaxRefNumber") or it.get("PaxRef")
                )
                pax_ref_int = None
                try:
                ...
                code = (
                    ...
                    or it.get("BaggId")
                    or it.get("SeatId")
                    or it.get("OtherSSRId")
                    or ""
                )
                desc = it.get("Description") or str(code)
                price = (
                    Decimal(str(it.get("Amount") or it.get("SeatAmount") or 0))
                    if "Amount" in it or "SeatAmount" in it
                    else Decimal("0")
                )
        ...
        if payment_amount > 0:
            booking.final_amount = (
                booking.final_amount or Decimal("0")
            ) + payment_amount
        ...
        try:
            ...
            logger.info(
                f"SSR services added SMS notification queued for booking {booking.id}"
            )
...
def process_reschedule_success(
    booking: Booking,
    flight_booking: FlightBooking,
    airiq_resp: dict,
    remarks: str = "",
    pnr_index: int = None,
    old_airiq_pnr: str = None,
) -> Dict:
    """
    ...
    try:
        ...
        reschedule_status = (
            flight_booking.airiq_response_data.get("reschedule_status", {}) or {}
        )
        ...
        if len(airiq_pnrs) > 1:
            for idx in range(len(airiq_pnrs)):
                pnr_key = f"pnr_{idx}"
                if (
                    pnr_key not in reschedule_status
                    or reschedule_status[pnr_key].get("status") != "success"
                ):
        ...
        if new_pnr:
            ...
            if flight_booking.airiq_pnrs:
                ...
                if pnr_index is not None and 0 <= pnr_index < len(
                    flight_booking.airiq_pnrs
                ):
        ...
        try:
            ...
            if flight_booking.departure_date:
                new_departure_str = flight_booking.departure_date.strftime(
                    "%B %d, %Y %I:%M %p"
                )

            send_booking_sms_task.delay(
                notification_type="FLIGHT_BOOKING_RESCHEDULED",
                params={"booking_id": booking.id, "new_departure": new_departure_str},
            )
...
def record_reschedule_failure(
    flight_booking: FlightBooking, old_airiq_pnr: str, error: str, pnr_index: int = None
) -> None:
    """
    ...
    try:
        ...
        for idx, itin_detail in enumerate(itinerary_details):
            ...
            if traveller_items:
                ...
                first_traveller = (
                    traveller_items[0]
                    if isinstance(traveller_items, list)
                    else traveller_items
                )
            ...
            if idx == 0:
                ...
                if (
                    base_origin == route_info["onward_destination"]
                    or not route_info["return_origin"]
                ):
                    route_info["return_origin"] = base_origin
                    route_info["return_destination"] = base_dest
...
def process_multi_pnr_reschedule_availability(
    flight_booking: FlightBooking, reschedule_requests: list, airiq_service
) -> Dict:
    """
...
def process_multi_pnr_reschedule_confirm(
    flight_booking: FlightBooking,
...
def process_reschedule_refund(
    booking: Booking, refund_amount: Decimal, reschedule_details: dict
) -> Dict:
    """
    ...
    try:
        ...
        original_payment = (
            booking.booking_payment.filter(is_transaction_success=True)
            .order_by("-id")
            .first()
        )
        ...
        if (
            original_payment.payment_medium == "Idbook"
            or original_payment.payment_type == "WALLET"
        ):
            # Refund to wallet
...
class FlightBookingAuthManager:
    ...
    def validate_user_eligibility(self) -> Tuple[bool, str, User]:
        ...
        if existing_user:
            ...
            if existing_user.is_active:
                return (
                    True,
                    "Existing account - email verification required",
                    existing_user,
                )
            else:
                return (
    ...
    def create_guest_user(self) -> User:
        ...
        pax_list = (
            self.request_data.get("passengers")
            or self.request_data.get("PaxDetailsInfo")
            or [{}]
        )
        passenger_data = pax_list[0] if pax_list else {}

        first_name = passenger_data.get("first_name") or passenger_data.get(
            "FirstName", ""
        )
        last_name = passenger_data.get("last_name") or passenger_data.get(
            "LastName", ""
        )
...
class FlightBookingProcessor:
    ...
    def validate_booking_data(self) -> bool:
        ...
        if len(passengers) != expected_count:
            self.errors.append(
                f"Expected {expected_count} passengers, got {len(passengers)}"
            )
        ...
        if gst_info and gst_info.get("gst_number"):
            ...
            for field in required_gst_fields:
                if not gst_info.get(field):
                    self.errors.append(
                        f"GST {field} is required when GST number is provided"
                    )
    ...
    def calculate_pricing(self) -> Dict[str, Decimal]:
        ...
        if base_amount <= 0:
            raise ValueError(
                "Invalid or missing total_amount. Flight pricing data is required for booking creation."
            )
    ...
    @transaction.atomic
    def create_booking(
        self, airiq_response: dict = None
    ) -> Tuple[Booking, FlightBooking]:
        ...
        flight_booking_data = {
            "flight_no": self.booking_data["flight_segments"][0].get(
                "flight_number", ""
            ),
            "airline_code": self.booking_data["flight_segments"][0].get(
                "flight_number", ""
            )[:2],
            ...
            if self.booking_data.get("block_pnr") and airiq_response.get("HoldExpiry"):
                ...
                try:
                    ...
                    if isinstance(hold_expiry, str):
                        flight_booking_data["hold_expires_at"] = parse_datetime(
                            hold_expiry
                        )
                    else:
                    ...
                    flight_booking_data["hold_expires_at"] = timezone.now() + timedelta(
                        hours=24
                    )
        ...
        if booking.user and self.booking_data.get("contact"):
            ...
            if not contact_name:
                ...
                if passengers:
                    ...
                    first_name = first_pax.get("first_name") or first_pax.get(
                        "FirstName", ""
                    )
                    last_name = first_pax.get("last_name") or first_pax.get(
                        "LastName", ""
                    )
        ...
        BookingMetaInfo.objects.create(
            ...
            booking_confirmed_date=(
                timezone.now() if booking.status == "confirmed" else None
            ),
        )
    ...
    def create_passengers(
        self, booking: Booking, flight_booking: FlightBooking
    ...
    def create_ancillary_services(
        self, flight_booking: FlightBooking, passengers: List[FlightPassenger]
    ) -> List[FlightAncillaryService]:
                ...
                if passenger:
                    service = FlightAncillaryService.objects.create(
                        ...
                        service_price=Decimal(
                            "0.00"
                        ),  # This would come from AirIQ response
                    )
    ...
    def _send_booking_notifications(self, booking: Booking):
        ...
        try:
            ...
            logger.error(
                f"Error sending booking notifications for {booking.id}: {str(e)}"
            )
...
class FlightCancellationManager:
    ...
    def calculate_refund_amount(
        ...
        refund_amount, refund_details = calculate_refund_amount(
            total_payment_made=payment_details, applicable_policy=cancellation_policy
        )
    ...
    def process_cancellation(self, remarks: str = "") -> Tuple[bool, dict]:
        ...
        try:
            ...
            cancellation_details = {
                ...
                "cancelled_by": (
                    self.booking.user.email if self.booking.user else "System"
                ),
    ...
    def process_refund(
        self, refund_amount: Decimal, cancellation_details: dict
    ) -> Tuple[bool, str, dict]:
        ...
        if payment_details.payment_medium == "Idbook":  # Wallet payment
            return refund_wallet_payment(
                self.booking, refund_amount, cancellation_details
            )
        else:
            # For other payment methods (PhonePe, etc.), use existing refund mechanism
            return self.process_gateway_refund(
                payment_details, refund_amount, cancellation_details
            )

    def process_gateway_refund(
        self,
        payment_details: BookingPaymentDetail,
    ...
    def _send_cancellation_notifications(self, refund_amount: Decimal):
        ...
        try:
            ...
            logger.info(
                f"Cancellation notifications sent for flight booking {self.booking.id}"
            )

        except Exception as e:
            logger.error(
                f"Error sending cancellation notifications for {self.booking.id}: {str(e)}"
            )
...
def send_flight_ticket_notification(booking_id: int):
    ...
    try:
        ...
        booking = Booking.objects.filter(id=booking_id, booking_type="FLIGHT").first()
...
def calculate_flight_booking_gst(
    amount: Decimal,
    is_business: bool = False,
    origin_state: str = "DL",
    destination_state: str = "DL",
) -> dict:
    """
...
def validate_flight_booking_eligibility(
    user, booking_data: dict
) -> Tuple[bool, List[str]]:
    """
...
def get_flight_booking_stats(user, date_from=None, date_to=None) -> dict:
    ...
    bookings = Booking.objects.filter(
        booking_type="FLIGHT", created__date__range=[date_from, date_to]
    )
    ...
    top_routes = (
        bookings.filter(flight_booking__isnull=False)
        .values("flight_booking__flying_from", "flight_booking__flying_to")
        .annotate(booking_count=Count("id"), total_revenue=Sum("final_amount"))
    ...
    for i in range(6):  # Last 6 months
        ...
        ).aggregate(bookings=Count("id"), revenue=Sum("final_amount"))
