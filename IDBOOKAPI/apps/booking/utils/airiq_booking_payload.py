...
def build_airiq_booking_payload(
    ...
    payload = {
        ...
        "PaxDetailsInfo": _build_passenger_details(
            user_data.get('passengers', [])
        ),
        
        # === CONTACT DETAILS (User Input Required) ===
        "AddressDetails": _build_contact_details(
            user_data.get('contact', {})
        ),
        
        # === GST INFO (Optional User Input) ===
        "GSTInfo": _build_gst_info(
            user_data.get('gst_info', {})
        ),
        
        # === FREQUENT FLYER (Optional User Input) ===
        "FFNumberInfo": _build_frequent_flyer_info(
            user_data.get('frequent_flyer', [])
        ),
...
def _build_itinerary_info(flight_booking, user_data: Dict, selected_flight: Dict) -> List[Dict]:
    ...
    itinerary_info = {
        ...
        "PaymentInfo": [{
            "TotalAmount": str(user_data.get('total_amount', 0))
        }]
    }
...
def _build_passenger_details(passengers: List[Dict]) -> List[Dict]:
    ...
    for i, passenger in enumerate(passengers):
        pax_detail = {
            ...
            "PassportIssuedDate": _format_date(passenger.get('passport_issued_date', '')),
