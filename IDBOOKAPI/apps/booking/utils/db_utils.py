...
def get_user_based_booking(user_id, booking_id):
    booking = Booking.objects.filter(id=booking_id, user__id=user_id).first()
    return booking
...
def get_booked_room(check_in, check_out, is_slot_price_enabled=False):
    ...
    ##            on_hold_end_time__gte=on_hold_end_time)
        ...
        booked_hotel = Booking.objects.filter(
            Q(
                status="confirmed",
                hotel_booking__confirmed_checkin_time__date__lt=check_out,
    ...
    booked_hotel = booked_hotel.values(
        "hotel_booking__confirmed_property_id",
        "hotel_booking__room_id",
        "hotel_booking__confirmed_room_details",
    )
    return booked_hotel


def check_room_booked_details(
    check_in, check_out, property_id, is_slot_price_enabled=False, booking_id=None
):
    on_hold_end_time = datetime.now(timezone("UTC"))
    if is_slot_price_enabled:
        booked_hotel = Booking.objects.filter(
...
def get_booked_hotel_booking(check_in, check_out, property_id):
    ...
    booked_hotel = booked_hotel.filter(
        Q(status="confirmed")
        | Q(status="on_hold", on_hold_end_time__gte=on_hold_end_time)
    )
...
def get_booking_based_tax_rule(booking_type):
    tax_rules = TaxRule.objects.filter(booking_type=booking_type).values(
        "id", "math_compare_symbol", "tax_rate_in_percent", "amount1", "amount2"
    )
    return tax_rules
...
def check_user_used_coupon(coupon_code, user_id):
    coupon_used = Booking.objects.filter(
        user__id=user_id, coupon_code=coupon_code, status="confirmed"
    ).exists()
    return coupon_used
...
def create_booking_payment_details(booking_id, append_id):
    ...
    while True:
        ...
        if not is_merchant_transactionid_exist(merchant_transaction_id):
            break
        merchant_transaction_id = None
...
def is_merchant_refundid_exist(merchant_refund_id):
    return BookingRefundLog.objects.filter(
        merchant_refund_id=merchant_refund_id
    ).exists()
...
def get_refund_log_by_merchant_id(merchant_refund_id):
    try:
        ...
        return (
            BookingRefundLog.objects.filter(merchant_refund_id=merchant_refund_id)
            .order_by("-created")
            .first()
        )
...
def refund_create_booking_payment_details(
    merchant_transaction_id, booking_payment_details: dict
):
    ...
    if refund_log and refund_log.booking:
        ...
        print(
            f"Error: Cannot create payment detail for {merchant_transaction_id}, booking not found in refund logs"
        )
        return None
...
def get_property_based_review_count(property_id):
    total_review_count = Review.objects.filter(
        property_id=property_id, overall_rating__isnull=False
    ).count()
...
def change_onhold_status():
    try:
        ...
        result = Booking.objects.filter(
            status="on_hold", on_hold_end_time__lt=on_hold_end_time
...
def get_total_property_confirmed_booking(property_id):
    total_confirmed_booking = Booking.objects.filter(
        hotel_booking__confirmed_property=property_id, status="confirmed"
    ).count()
    return total_confirmed_booking


def get_overall_booking_rating(property_id):
    review_obj = Review.objects.filter(property_id=property_id, active=True)
    ...
    agency_review_list = review_obj.annotate(
        booking_experience=Cast(
            KT("agency_review__booking_experience_rating"), FloatField()
        ),
        cancellation_experience=Cast(
            KT("agency_review__cancellation_experience_rating"), FloatField()
        ),
        search_property_experience=Cast(
            KT("agency_review__search_property_experience_rating"), FloatField()
        ),
...
def check_booking_reference_code(reference_code):
    is_exist = Booking.objects.filter(reference_code=reference_code).exists()
    return is_exist


def check_booking_confirmation_code(confirmation_code):
    is_exist = Booking.objects.filter(confirmation_code=confirmation_code).exists()
    return is_exist
...
def get_booking_commission(booking_id):
    booking_commission = BookingCommission.objects.filter(booking=booking_id).first()
...
def save_invoice_to_database(booking, payload_json, invoice_number):
    ...
    try:
        ...
        if not billed_by:
            print(
                "Error: No active business found. Cannot create invoice without billed_by."
            )
            return None
        ...
        for item in items:
            ...
            if (
                "room_amount_with_discount" in item
                and item["room_amount_with_discount"]
            ):
        ...
        invoice = Invoice(
            ...
            due_date=(
                datetime.fromisoformat(payload.get("dueDate"))
            ...
            next_schedule_date=(
                datetime.fromisoformat(payload.get("nextScheduleDate"))
                if payload.get("nextScheduleDate")
                else None
            ),
...
def update_invoice_in_database(invoice_number, payload, booking):
    ...
    try:
        ...
        invoice.pro_member_discount = payload.get(
            "pro_member_discount", invoice.pro_member_discount
        )
        ...
        if "nextScheduleDate" in payload:
            invoice.next_schedule_date = (
                datetime.fromisoformat(payload["nextScheduleDate"])
                if payload["nextScheduleDate"]
                else None
            )
...
def get_hotelier_amount_payout(property_id):
    booking_obj = Booking.objects.filter(
        hotel_booking__confirmed_property=property_id, status="completed"
    ).prefetch_related("commission_info")
    booking_obj = booking_obj.filter(
        commission_info__booking__isnull=False,
        commission_info__is_payment_approved=True,
    )
    booking_obj = booking_obj.filter(
        Q(commission_info__payout_status="PENDING")
...
def update_payout_booking(booking_ids, payout_status, latest_payout_reference_id):
    BookingCommission.objects.filter(booking_id__in=booking_ids).update(
