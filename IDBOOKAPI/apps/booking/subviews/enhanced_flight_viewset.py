...
from ..utils.flight_booking_utils import (
...
class EnhancedFlightBookingViewSet(
    ...
    def get_flight_booking(self, booking_id, guest_token=None):
        ...
        if not guest_token and hasattr(self, "request"):
            guest_token = self.request.data.get(
                "guest_token"
            ) or self.request.query_params.get("guest_token")
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "session_id": openapi.Schema(
                    ...
                    description="Optional: Pricing session ID from search",
                ),
                "TrackId": openapi.Schema(
                    type=openapi.TYPE_STRING, description="AirIQ Track ID for pricing"
                ),
                ...
                "TripType": openapi.Schema(
                    type=openapi.TYPE_STRING, enum=["O", "R"], default="O"
                ),
                "BaseOrigin": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Origin airport code"
                ),
                "BaseDestination": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Destination airport code"
                ),
                ...
                "PaxDetailsInfo": openapi.Schema(
                    ...
                    items=openapi.Items(
                        ...
                        properties={
                            ...
                            "PaxType": openapi.Schema(
                                type=openapi.TYPE_STRING, enum=["ADT", "CHD", "INF"]
                            ),
                ...
                "AddressDetails": openapi.Schema(
                    ...
                    properties={
                        "CountryCode": openapi.Schema(
                            type=openapi.TYPE_STRING, default="91"
                        ),
                        "ContactNumber": openapi.Schema(type=openapi.TYPE_STRING),
                        "EmailID": openapi.Schema(
                ...
                "guest_booking": openapi.Schema(
                    type=openapi.TYPE_BOOLEAN, default=False
                ),
                "otp": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="OTP for guest booking verification",
                ),
                "company_id": openapi.Schema(
                    type=openapi.TYPE_INTEGER,
                    description="Company ID (required for corporate users)",
                ),
            },
        ),
        responses={
                    ...
                    properties={
                        ...
                        "data": openapi.Schema(
                            ...
                            properties={
                                ...
                                "booking_reference": openapi.Schema(
                                    type=openapi.TYPE_STRING
                                ),
                                ...
                                "payment_expires_at": openapi.Schema(
                                    type=openapi.TYPE_STRING, format="datetime"
                                ),
                                "payment_lock_duration": openapi.Schema(
                                    type=openapi.TYPE_INTEGER, description="Minutes"
                                ),
                            },
                        ),
                    },
    ...
    def create_booking(self, request):
        ...
        try:
            ...
            is_eligible, auth_message, auth_user = (
            ...
            if request.user and request.user.is_authenticated:
                ...
                if not request.data.get("otp"):
                    success, message, verification_data = (
                        auth_manager.initiate_guest_booking()
                    )
                    if success:
                ...
                success, message, guest_user = auth_manager.verify_guest_booking_otp(
                    request.data["otp"]
                )
                if not success:
                    return self.get_error_response(
            ...
            booking_data = self._prepare_booking_data_from_request(
                request.data, pricing_validation
            )
            ...
            if booking_user:
                ...
                if is_corporate:
                    ...
                    try:
                        ...
                        if booking_user.company_id and booking_user.company_id != int(
                            company_id
                        ):
                            return self.get_error_response(
            ...
            response_data = {
                ...
                "booking_details": {
                    ...
                    "departure_date": (
                        flight_booking.departure_date.isoformat()
                        if flight_booking.departure_date
                        else None
                    ),
                    "flight_trip": flight_booking.flight_trip,
                    "passenger_count": booking.adult_count
                    + booking.child_count
                ...
                "amount_breakdown": {
                    ...
                    "gross_amount": float(
                        pricing_validation.get("gross_amount", booking.final_amount)
                    ),
                    ...
                    "total_discount": float(
                        pricing_validation.get("total_discount", 0)
                    ),
                    "final_amount": float(
    ...
    @action(
        detail=False, methods=["get"], url_path=r"(?P<booking_id>\d+)/airiq/retrieve"
    )
    def airiq_retrieve_booking(self, request, booking_id=None):
        try:
            ...
            body = {
                "Retrieveresponse": resp.get("Retrieveresponse")
                or resp.get("Retriveresponse")
                or resp.get("Bookingresponse")
                or resp,
                "Status": resp.get("Status")
                or resp.get("ResponseStatus")
                or {"ResultCode": "1", "Error": ""},
            }
            return self.get_response(
                data=body,
            ...
            return self.get_error_response(
                message=str(e), status="error", status_code=status.HTTP_404_NOT_FOUND
            )
            ...
            self.log_error(
                f"Unexpected error in AirIQ retrieve for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path=r"(?P<booking_id>\d+)/airiq/track-status",
    )
    def airiq_track_status(self, request, booking_id=None):
        try:
            ...
            resp = airiq_service.track_booking_status(
            ...
            try:
                ...
                if isinstance(resp, dict):
                    ar = (
                        resp.get("Retrieveresponse")
                        or resp.get("Retriveresponse")
                        or resp.get("TrackStatusResponse")
                        or {}
                    )
                    itins = (
                        ar.get("ItinearyDetails") or ar.get("ItineraryDetails") or []
                    )
                    if isinstance(itins, list) and itins:
                        items = itins[0].get("Item") or []
                        if items:
                            retr_pnr = items[0].get("AirIqPNR") or items[0].get(
                                "AiriqPNR"
                            )
                ...
                if retr_needed or not flight_booking.ticket_numbers:
                    if flight_booking.airiq_pnr:
                        retr_resp = airiq_service.get_booking_details(
                            airiq_pnr=flight_booking.airiq_pnr
                        )
                        self._update_booking_from_retrieve_response(
                            booking, flight_booking, retr_resp
                        )
            except Exception as e:
                logger.warning(
                    f"Track-status enrichment failed for booking {booking_id}: {e}"
                )
            ...
            if not track_block:
                possible_blocks = [
                    resp.get("TrackStatusresponse"),
                    resp.get("Retrieveresponse"),
                    resp.get("Retriveresponse"),
                    resp,
                ]
                itins = []
                for blk in possible_blocks:
                    if isinstance(blk, dict):
                        itins = (
                            blk.get("ItinearyDetails")
                            or blk.get("ItineraryDetails")
                            or []
                        )
                        if itins:
                            break
                track_block = {"ItinearyDetails": itins}
            body = {"Status": final_status, "TrackStatusresponse": track_block}
            return self.get_response(
                data=body,
            ...
            self.log_error(
                f"AirIQ track-status error for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
            ...
            self.log_error(
                f"Unexpected error in AirIQ track-status for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
                message="An unexpected error occurred",
                status="error",
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                "AirIqPNR": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Override AirIQ PNR (optional)",
                ),
                "AirlinePNR": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Override Airline PNR (optional)",
                ),
            },
        ),
        responses={
            200: openapi.Response(description="SSR options with TrackId and price data")
        },
    )
    @action(
        detail=True,
        methods=["post"],
        url_path="ancillary/get-ssr",
    ...
    def get_ssr_options(self, request, pk=None):
        try:
            ...
            ssr_resp = airiq_service.get_ssr_services(
                airiq_pnr=airiq_pnr, airline_pnr=airline_pnr
            )
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "TracKID": openapi.Schema(
                    type=openapi.TYPE_STRING, description="TrackId from GetSSR response"
                ),
                "AirIqPNR": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Override AirIQ PNR (optional)",
                ...
                "MealsSSR": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_OBJECT),
                ),
                "BaggSSR": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_OBJECT),
                ),
                "SeatsSSR": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_OBJECT),
                ),
                "OtherSSR": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_OBJECT),
                ),
                "Payment": openapi.Schema(
                    ...
                    items=openapi.Items(
                        type=openapi.TYPE_OBJECT,
                        properties={
                            "PaymentMode": openapi.Schema(
                                type=openapi.TYPE_STRING, default="T"
                            ),
                            "Amount": openapi.Schema(type=openapi.TYPE_STRING),
                        },
                    ),
                ),
                "payment_channel": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=["WALLET", "PHONE PAY"],
                    description="Payment method",
                ),
                "redirect_url": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Redirect URL for PhonePe payment",
                ),
            },
        ),
        responses={
            200: openapi.Response(
                description="Updated booking snapshot and AirIQ response"
            )
        },
    )
    @action(
            ...
            try:
                if payment_arr and isinstance(payment_arr, list):
                    payment_amount = Decimal(
                        str((payment_arr[0] or {}).get("Amount") or 0)
                    )
            except Exception:
                payment_amount = Decimal("0")
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path="ancillary/phonepe-callback",
        permission_classes=[],
    )
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path="reschedule/phonepe-callback",
        permission_classes=[],
    )
    def reschedule_phonepe_callback(self, request):
        ...
        try:
            from apps.booking.utils.flight_payment_utils import (
                process_reschedule_phonepe_callback,
            )
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path="ticket/phonepe-callback",
        permission_classes=[AllowAny],
    )
    def ticket_phonepe_callback(self, request):
        ...
        try:
            ...
            from apps.booking.utils.flight_payment_utils import (
                process_ticket_issuance_phonepe_callback,
            )
    ...
    def _normalize_itinerary_flights(
        self, itin_list: list, default_token: str = None, flights_override: dict = None
    ) -> list:
    ...
    def _build_airiq_booking_request(
        self, request_data: dict, pricing_validation: dict = None
    ) -> dict:
        ...
        itin_norm = self._normalize_itinerary_flights(
            itin_list,
            default_token=pricing_token,
            flights_override=flights_grouped if flights_grouped else None,
        )
        ...
        airiq_struct = {
            ...
            "TrackId": track_from_price
            or request_data.get("TrackId")
            or request_data.get("TrackID")
            or "",
        }
        return airiq_struct

    def _extract_amounts_from_pricing_response(
        self, pricing_response: dict, request_data: dict
    ) -> dict:
        ...
        try:
            ...
            for row in fdesc:
                ...
                for tx in taxes:
                    ...
                    tax_breakdown[code] = tax_breakdown.get(code, 0.0) + (
                        amt * (pax_count if pax_count > 0 else 1)
                    )
    ...
    def _extract_pricing_from_request(self, request_data: dict) -> dict:
        ...
        try:
            ...
            pricing_token = (
                itinerary_flights[0].get("Token", "") if itinerary_flights else ""
            )
            ...
            for flight_info in itinerary_flights:
                ...
                try:
                    total_amount += float(
                        ta
                        if ta not in (None, "")
                        else (ga if ga not in (None, "") else (ba or 0))
                    )
                except Exception:
                    pass
                try:
                ...
                for tax in p.get("Taxes") or []:
                    ...
                    amt = tax.get("Amount")
                    try:
                        tax_breakdown[code] = tax_breakdown.get(code, 0.0) + float(
                            amt or 0
                        )
                    except Exception:
                        pass
    ...
    def _get_fare_rules_response(self, request_data: dict) -> dict:
        ...
        try:
            ...
            flights_info = (
                first_itin.get("FlighstInfo") or first_itin.get("FlightsInfo") or []
            )
            flight_ids = []
            ...
            if not flight_ids:
                return None
            resp = airiq_service.get_fare_rules(
                flight_ids=flight_ids, track_id=track_id
            )
            status_block = resp.get("Status") or resp.get("ResponseStatus") or {}
    ...
    def _validate_current_pricing(self, request_data: dict, session_data: dict) -> dict:
        ...
        try:
            ...
            flight_details = (
                itinerary_flights[0].get("FlighstInfo", []) if itinerary_flights else []
            )
            base_amount = (
                itinerary_flights[0].get("PaymentInfo")[0].get("BaseAmount", 0)
                if itinerary_flights
                else 0
            )
            gross_amount = (
                itinerary_flights[0].get("PaymentInfo")[0].get("GrossAmount", 0)
                if itinerary_flights
                else 0
            )
            ...
            try:
                ...
                if price_itinerary_info:
                    availability_response = price_itinerary_info[0].get(
                                ...
                                for passenger in itinerary["PassengerDetails"]:
                                    if "SegmentInformation" in passenger:
                                        ...
                                        if "MonetaryDetail" in segment_info:
                                            monetary_detail = segment_info[
                                                "MonetaryDetail"
                                            ]
                                            current_total = float(
                                                monetary_detail.get("GrossAmount", 0)
                                            )
                                            basic_amount = float(
                                                monetary_detail.get("BasicAmount", 0)
                                            )
                                            break
                                if current_total > 0:
                                    break
            ...
            return {"success": False, "message": "Pricing validation failed"}
    ...
    def _check_agent_balance(self, booking_amount: float) -> dict:
        ...
        try:
            ...
            if not balance_response or "success" not in balance_response:
                ...
                return {"success": False, "message": "Unable to verify account balance"}

            if not balance_response["success"]:
                logger.error(
                    f"AirIQ balance check failed: {balance_response.get('message', 'Unknown error')}"
                )
                return {
                    "success": False,
                    "message": balance_response.get(
            ...
            logger.info(
                f"Agent balance details - Topup: {topup_balance}, Credit: {credit_balance}, Total: {available_balance}"
            )
            ...
            if (
                ...
                and topup_balance < required_amount
                and credit_balance < required_amount
            ):
                logger.warning(
                    f"Insufficient agent balance: {available_balance} < {required_amount}"
                )
                return {
                    "success": False,
    ...
    def _extract_gst_from_airiq_response(self, monetary_detail: dict) -> dict:
        ...
        total_gst = (
            gst_breakdown["cgst_amount"]
            + gst_breakdown["sgst_amount"]
            + gst_breakdown["igst_amount"]
        )
    ...
    def _extract_gst_from_new_response_structure(
        self, basic_amount: float, gross_amount: float, tax_breakdown: dict
    ) -> dict:
    ...
    def _prepare_booking_data_from_request(
        self, request_data: dict, pricing_validation: dict
    ) -> dict:
        ...
        basic_amount = pricing_validation.get(
            "basic_amount", pricing_validation["final_amount"]
        )
        pricing_token = pricing_validation.get("pricing_token", "")
    ...
    def _handle_block_pnr_booking(
        ...
        try:
            ...
            airiq_request = self._build_airiq_booking_request(
                request.data, pricing_validation
            )

            # Convert AirIQ request format to format expected by airiq_service.create_booking
            booking_data = self._convert_airiq_request_to_booking_data(
                airiq_request, request.data, pricing_validation
            )
            ...
            try:
                airiq_response = airiq_service.create_booking(
                    booking_data=booking_data, track_id=track_id, block_pnr=True
                )
            except AirIQException as e:
            ...
            with transaction.atomic():
                ...
                flight_booking.airiq_track_id = booking_details.get(
                    "track_id", track_id
                )
                ...
                self._update_booking_from_airiq_response(
                    booking, flight_booking, airiq_response
                )
            ...
            response_data = {
                ...
                "total_amount": float(booking.final_amount),
                "hold_expires_at": (
                    flight_booking.hold_expires_at.isoformat()
                    if flight_booking.hold_expires_at
                    else None
                ),
    ...
    def _convert_airiq_request_to_booking_data(
        self, airiq_request: dict, request_data: dict, pricing_validation: dict
    ) -> dict:
        """Convert AirIQ request format to format expected by airiq_service.create_booking"""
        ...
        return {
            ...
            "total_amount": (
                pricing_validation.get("final_amount", 0) if pricing_validation else 0
            ),
            "seats": seats,
            "meals": meals,
            "baggage": baggage,
    ...
    def _update_booking_from_airiq_response(
        self, booking: Booking, flight_booking: FlightBooking, airiq_response: dict
    ):
        ...
        try:
            ...
            if ticketing_time_limit:
                ...
                if isinstance(flight_booking.airiq_response_data, dict):
                    flight_booking.airiq_response_data["ticketing_time_limit"] = (
    ...
    def _save_ticket_response(
        self, booking: Booking, flight_booking: FlightBooking, ticket_response: dict
    ):
        ...
        try:
            ...
            logger.error(
                f"Error saving ticket response for booking {booking.id}: {str(e)}"
            )
    ...
    def _create_booking_local_only(
        self,
        processor: FlightBookingProcessor,
        request_data: dict,
        pricing_validation: dict,
        company=None,
    ) -> tuple:
        ...
        airiq_req_struct = self._build_airiq_booking_request(
            request_data, pricing_validation
        )
        ...
        try:
            raw_price = (
                request_data.get("pricing_response")
                or request_data.get("pricing_info")
                or {}
            )
            raw_seatmap = (
                request_data.get("avail_seat_map_response")
                or request_data.get("SeatMapResponse")
                or {}
            )
            if raw_price:
        ...
        if itinerary_flights and "FlighstInfo" in itinerary_flights[0]:
            ...
            if departure_str:
                try:
                    flight_booking.departure_date = datetime.strptime(
                        departure_str, "%d %b %Y %H:%M"
                    )
                except ValueError:
                    pass
        ...
        if (
            request_data.get("TripType", "O") == "R"
            and len(itinerary_flights) > 1
            and "FlighstInfo" in itinerary_flights[1]
            and itinerary_flights[1]["FlighstInfo"]
        ):
            ...
            if ret_dep:
                try:
                    flight_booking.return_date = datetime.strptime(
                        ret_dep, "%d %b %Y %H:%M"
                    )
                except ValueError:
                    pass
        ...
        try:
            booking.final_amount = Decimal(
                str(
                    pricing_validation.get(
                        "payable_amount", pricing_validation["final_amount"]
                    )
                )
            )
        except Exception:
        ...
        if (
            "total_discount" in pricing_validation
            and pricing_validation["total_discount"] > 0
        ):
        ...
        self._create_passenger_records(
            booking, flight_booking, processor.booking_data["passengers"]
        )
    ...
    def _prepare_booking_data(self, request_data: dict, session_data: dict) -> dict:
        ...
        booking_data = {
            ...
            # Pricing data
            "total_amount": booking_total.get(
                "final_total", pricing_breakdown.get("gross_amount", 0)
            ),
    ...
    def _create_booking_with_airiq(
        self, processor: FlightBookingProcessor, session_data: dict, block_pnr: bool
    ) -> tuple:
        ...
        # Create local booking records
        booking, flight_booking = processor.create_booking(
            airiq_response=airiq_response
        )
        ...
        flight_booking.selected_flight_data = processor.booking_data[
        ...
        if block_pnr:
            ...
            try:
                ...
                hold_expiry_str = flight_booking.hold_expires_at.strftime(
                    "%B %d, %Y %I:%M %p"
                )
                send_booking_sms_task.delay(
                    notification_type="FLIGHT_HOLD_REQUESTED",
                    params={"booking_id": booking.id, "hold_expiry": hold_expiry_str},
                )
                self.log_info(
                    f"Hold booking SMS notification queued for booking {booking.id}"
                )
            except Exception as e:
    ...
    def _extract_airiq_booking_details(self, airiq_response: dict) -> dict:
        ...
        if itinerary_details:
            ...
            airline_pnr = (
                airline_pnr
                or details.get("AirlinePNR", "")
                or details.get("CRSPNR", "")
            )
            # Normalize NA values
            if isinstance(airline_pnr, str) and airline_pnr.strip().upper() in (
                "N/A",
                "NA",
                "NULL",
            ):
    ...
    def _create_passenger_records(
        self, booking: Booking, flight_booking: FlightBooking, passengers: list
    ):
        ...
        for passenger in passengers:
            ...
            passport_expiry = self._convert_date_string(
                passenger.get("passport_expiry")
            )
            passport_issued_date = self._convert_date_string(
            ...
            passenger_record = FlightPassenger(
                ...
                infant_with_passenger=self._convert_infant_ref(
                    passenger.get("infant_ref")
                ),
            )
    ...
    def _convert_date_string(self, date_string):
        ...
        if isinstance(date_string, str):
            ...
            raise ValueError(
                f"Unable to parse date: {date_string}. Expected formats: DD/MM/YYYY, YYYY-MM-DD, DD-MM-YYYY, YYYY/MM/DD, MM/DD/YYYY"
            )
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "booking_id", openapi.IN_PATH, type=openapi.TYPE_INTEGER, required=True
            )
        ],
    )
    ...
    def get_booking_status(self, request, pk=None):
        ...
        try:
            ...
            if flight_booking.airiq_track_id:
                try:
                    airiq_status = airiq_service.track_booking_status(
                        flight_booking.airiq_track_id
                    )
                except AirIQException as e:
                    logger.warning(
                        f"Failed to get AirIQ status for booking {pk}: {str(e)}"
                    )
            ...
            airiq_booking = (
                airiq_raw.get("Bookingresponse") if isinstance(airiq_raw, dict) else {}
            ) or {}
            ...
            response_data = {
                ...
                "pnrs": {
                    ...
                    "airiq_track_ids": getattr(flight_booking, "airiq_track_ids", [])
                    or [],
                },
                ...
                "amount_details": {
                    ...
                    "balance_due": float(
                        booking.final_amount - booking.total_payment_made
                    ),
                },
                ...
                "airiq_raw": airiq_raw,
                "airiq_request": getattr(flight_booking, "airiq_request_data", {})
                or {},
            }
    ...
    def _update_booking_from_retrieve_response(
        self, booking: Booking, flight_booking: FlightBooking, retrieve_resp: dict
    ) -> None:
        """Update local booking records from AirIQ RetrieveBooking response (only missing/empty fields)."""
        try:
            retr = (
                retrieve_resp.get("Retrieveresponse")
                or retrieve_resp.get("Retriveresponse")
                or retrieve_resp.get("Bookingresponse")
                or {}
            )
            ...
            for itin in itins:
                ...
                if gross_amount_primary is None and trav:
                    mon = (trav[0].get("SegmentInformation") or {}).get(
                        "MonetaryDetail"
                    ) or {}
                    try:
            ...
            if (
                not flight_booking.ticket_numbers
                and "ticket_numbers" in locals()
                and ticket_numbers
            ):
            ...
            if not flight_booking.status or flight_booking.status in [
                "INITIATED",
                "PENDING_PAYMENT",
            ]:
            ...
            if (
                gross_amount_primary is not None
                and float(booking.final_amount or 0) == 0.0
            ):
                from decimal import Decimal
            ...
            if (
                flight_booking.status in ["CONFIRMED", "TICKETED"]
                ...
                try:
                    pax_qs = {
                        (
                            p.title.upper(),
                            p.first_name.strip().upper(),
                            p.last_name.strip().upper(),
                            str(p.date_of_birth) if p.date_of_birth else None,
                        ): p
                        for p in flight_booking.passengers.all()
                    }
                    for t in trav:
                        key = (
                            (t.get("Title") or "").upper(),
                            (t.get("FirstName") or "").strip().upper(),
                            (t.get("LastName") or "").strip().upper(),
                            None,
                        )
                        ...
                        if seg_items:
                            ...
                            from apps.booking.models import (
                                FlightAncillaryService as _FAS,
                            )
            ...
            logger.warning(
                f"Failed to update local booking from retrieve response: {e}"
            )
    ...
    def issue_ticket(self, request, booking_id=None):
        ...
        try:
            ...
            if flight_booking.status == "HELD":
                ...
                total_paid = (
                    booking.booking_payment.filter(
                        is_transaction_success=True
                ...
                if balance_due > 0.01:  # Allow small rounding differences
                    ...
                    payment_method = (
                        (request.data.get("payment_method") or "").strip().upper()
                    )
                    ...
                    if (
                        ...
                        try:
                            ...
                            ) and request.user.default_group in (
                                "CORP-ADMIN",
                                "CORP-EMP",
                                "CORPORATE-GRP",
                            ):
                            ...
                            can_pay, balance_info = check_wallet_balance_for_booking(
                                booking, request.user, company_id=company_id
                            )
                            if can_pay:
                                payment_method = "WALLET"
                        except Exception as e:
                            self.log_error(
                                f"Error checking wallet balance for default payment: {str(e)}"
                            )
                    ...
                    # Handle PhonePe payment
                        ...
                        from apps.authentication.utils.token_utils import (
                        ...
                        active_group = get_user_active_group(booking_user, request)
                        user_default_group = (
                            getattr(booking_user, "default_group", "") or ""
                        )
                        user_group = active_group or user_default_group

                        # Determine booking type prefix
                        if is_corporate_user(user_group) or (
                            booking.company_id and booking_user.company_id
                        ):
                            # Corporate booking
                        ...
                        logger.info(
                            f"Ticket issuance PhonePe payment - Payment data: {payment_data}"
                        )
                        print("=" * 80)
                        print(
                            "CREATING FLIGHTPAYMENTPROCESSOR FOR TICKET ISSUANCE PHONEPE PAYMENT"
                        )
                        print(f"Payment Data: {payment_data}")
                        print(
                            f"Transaction Type: {payment_data.get('transaction_type')}"
                        )
                        print("=" * 80)
                        ...
                        processor = FlightPaymentProcessor(
                            booking, booking_user, payment_data, request=request
                        )

                        # Verify transaction_type is set in processor
                        processor_transaction_type = processor.payment_data.get(
                            "transaction_type"
                        )
                        logger.info(
                            f"Processor payment_data transaction_type: {processor_transaction_type}"
                        )
                        print(
                            f"Processor payment_data transaction_type: {processor_transaction_type}"
                        )
                        ...
                        if processor_transaction_type != "ticket_issuance_payment":
                            logger.error(
                                f"ERROR: Transaction type mismatch! Expected 'ticket_issuance_payment', got '{processor_transaction_type}'"
                            )
                            print(
                                f"ERROR: Transaction type mismatch! Expected 'ticket_issuance_payment', got '{processor_transaction_type}'"
                            )
                    ...
                    # Handle Razorpay payment
                        ...
                        return self.get_response(
                            data={
                                ...
                                "transaction_id": payment_detail.merchant_transaction_id,
                                "payment_method": "RAZORPAY",
                                "name": getattr(booking_user, "name", None) or booking_user.email,
                                "email": booking_user.email,
                                "contact": getattr(booking_user, "mobile_number", "") or "",
            ...
            self.log_error(
                f"AirIQ error issuing ticket for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
    ...
    def cancel_booking(self, request, booking_id=None):
        ...
        try:
            ...
            guest_token = request.data.get("guest_token") or request.query_params.get(
                "guest_token"
            )
            booking, flight_booking = self.get_flight_booking(
                booking_id, guest_token=guest_token
            )
            ...
            remarks = request.data.get(
                "remarks", f"Cancellation requested by user {request.user.id}"
            )
            ...
            if not pnrs_to_cancel:
                ...
                if airiq_pnrs_list:
                    for pnr in airiq_pnrs_list:
                        if pnr and str(pnr).strip().upper() not in (
                            "N/A",
                            "NA",
                            "NULL",
                            "",
            ...
            if not pnrs_to_cancel:
                if (
                    ...
                    and flight_booking.airiq_pnr.strip().upper()
                    not in ("N/A", "NA", "NULL", "")
                ):
                    pnrs_to_cancel.append(
                        {
            ...
            for pnr_info in pnrs_to_cancel:
                ...
                try:
                    cancellation_response = airiq_service.cancel_booking(
                        airiq_pnr=airiq_pnr, flag=flag, remarks=remarks
                    )

                    # Extract penalty if present
                    penalty_amount_str = (cancellation_response or {}).get(
                        "PenalityAmount"
                    ) or "0"
                    try:
                    ...
                    cancellation_errors.append(
                        {"airiq_pnr": airiq_pnr, "error": error_msg}
                    )
                    all_cancellation_responses.append(
                        {
                            "airiq_pnr": airiq_pnr,
            ...
            successful_cancellations = [
                r for r in all_cancellation_responses if r.get("success")
            ]
            ...
            total_paid = (
                booking.booking_payment.filter(is_transaction_success=True).aggregate(
                    total=Sum("amount")
                )["total"]
                or 0
            )
            ...
            successful_penalty_total = sum(
                resp.get("penalty_amount", 0.0) for resp in successful_cancellations
            )
            ...
            if net_refund > 0:
                ...
                success, refund_status, refund_data = cancel_mgr.process_refund(
                    Decimal(str(net_refund)), cancellation_details
                )
                ...
                try:
                    if isinstance(refund_data, dict) and "refund_amount" in refund_data:
                        refund_summary["refund_merchant_transaction_id"] = (
                            refund_data.get("merchant_refund_id")
                        )
                except Exception:
                    pass
            else:
            ...
            self.log_info(
                ...
                extra={
                    ...
                    "cancellation_errors": (
                        cancellation_errors if cancellation_errors else None
                    ),
                },
            )
    ...
    @swagger_auto_schema(
        ...
        responses={
            ...
            400: openapi.Response(
                description="Bad request - booking not in HELD status or missing PNRs"
            ),
            404: openapi.Response(description="Booking not found"),
            500: openapi.Response(description="AirIQ service error"),
        },
    ...
    def cancel_hold(self, request, booking_id=None):
        ...
        try:
            ...
            try:
                ...
                self.log_error(
                    f"AirIQ hold cancel failed for booking {booking_id}: {str(e)}"
                )
                return self.get_error_response(
            ...
            if cancel_status == "SUCCESS" and result_code == "1":
                ...
                flight_booking.save(
                    update_fields=["status", "cancelled_at", "airiq_response_data"]
                )
            ...
            self.log_error(
                f"AirIQ error cancelling hold for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
            ...
            self.log_error(
                f"Unexpected error cancelling hold for booking {booking_id}: {str(e)}"
            )
            import traceback
            ...
            properties={
                "flights": openapi.Schema(
                    ...
                    items=openapi.Items(
                        type=openapi.TYPE_OBJECT,
                        properties={
                            "flight_date": openapi.Schema(
                                type=openapi.TYPE_STRING, description="YYYY-MM-DD"
                            ),
                            "departure_station": openapi.Schema(
                                type=openapi.TYPE_STRING,
                                description="3-letter IATA code",
                            ),
                            "arrival_station": openapi.Schema(
                                type=openapi.TYPE_STRING,
                                description="3-letter IATA code",
                            ),
                        },
                ...
                "flight_date": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="YYYY-MM-DD (for single flight/backward compatibility)",
                ),
                "departure_station": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="IATA origin (backward compatibility)",
                ),
                "arrival_station": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="IATA destination (backward compatibility)",
                ),
                "remarks": openapi.Schema(type=openapi.TYPE_STRING),
            },
        ),
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path=r"(?P<booking_id>\d+)/reschedule/availability",
    )
    def reschedule_availability(self, request, booking_id=None):
        try:
            booking, flight_booking = self.get_flight_booking(booking_id)

            # Get AirIqPNR: Check request first, then fallback to booking
            airiq_pnr = request.data.get("AirIqPNR") or request.data.get("airiq_pnr")
            if not airiq_pnr:
                # Fallback to booking
                airiq_pnrs = flight_booking.airiq_pnrs or []
                if airiq_pnrs:
                    airiq_pnr = airiq_pnrs[0]
                elif flight_booking.airiq_pnr:
                    airiq_pnr = flight_booking.airiq_pnr
                else:
                    return self.get_error_response(
                        message="AirIQ PNR missing in request and booking",
                        status="error",
                        status_code=status.HTTP_400_BAD_REQUEST,
                    )

            # Get TripType: Check request first, then fallback to booking
            trip_type = request.data.get("TripType") or request.data.get("trip_type")
            if not trip_type or trip_type not in ["O", "R", "Y"]:
                # Fallback to booking
            ...
            # Ensure default is "O" if still not valid
            if trip_type not in ["O", "R", "Y"]:
                trip_type = "O"

            # Get Remarks: Check request first, default to "Test"
            remarks = request.data.get("Remarks") or request.data.get("remarks", "Test")
            ...
            # Priority: 1. AvailInfo from request, 2. flights array, 3. single flight_date, 4. booking data
            flight_segments = []

            # Check if AvailInfo is provided directly in request (AirIQ format)
            avail_info = request.data.get("AvailInfo") or request.data.get("avail_info")
            if avail_info and isinstance(avail_info, list):
                for idx, avail in enumerate(avail_info, 1):
                    dep_station = avail.get("DepartureStation") or avail.get("departure_station")
                    arr_station = avail.get("ArrivalStation") or avail.get("arrival_station")
                    flight_date = avail.get("FlightDate") or avail.get("flight_date")
                    
                    if not all([dep_station, arr_station, flight_date]):
                        return self.get_error_response(
                            message=f"AvailInfo #{idx}: DepartureStation, ArrivalStation, and FlightDate are all required",
                            status="error",
                            status_code=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    try:
                        flight_date_str = str(flight_date).strip()
                        if "-" in flight_date_str:
                            ...
                            return self.get_error_response(
                                message=f"AvailInfo #{idx}: Invalid date format '{flight_date_str}'. Use YYYY-MM-DD or YYYYMMDD",
                                status="error",
                                status_code=status.HTTP_400_BAD_REQUEST,
                            )
                        ...
                        return self.get_error_response(
                            message=f"AvailInfo #{idx}: Invalid date format. Use YYYY-MM-DD or YYYYMMDD | Error: {str(e)}",
                            status="error",
                            status_code=status.HTTP_400_BAD_REQUEST,
                        )
                    
                    flight_segments.append({
                        "departure_station": str(dep_station).strip().upper(),
                        "arrival_station": str(arr_station).strip().upper(),
                        "flight_date": flight_date_fmt,
                    })
            # Check if flights array is provided (new format)
            elif "flights" in request.data and isinstance(request.data["flights"], list):
                for idx, flight in enumerate(request.data["flights"], 1):
                    if not all(
                        [
                            flight.get("flight_date"),
                            flight.get("departure_station"),
                            flight.get("arrival_station"),
                        ]
                    ):
                        return self.get_error_response(
                    ...
                    try:
                        ...
                        # Log for debugging
                        self.log_info(
                            f"Flight #{idx} date parsing: '{flight_date_str}' (len={len(flight_date_str)})"
                        )
                        ...
                        flight_date_fmt = dt.strftime("%Y%m%d")
                        self.log_info(
                            f"Flight #{idx} date converted: {flight_date_str} -> {flight_date_fmt}"
                        )

                    except ValueError as e:
                        self.log_error(
                            f"Flight #{idx} date parsing error: {str(e)} | Input: '{flight.get('flight_date')}'"
                        )
                        return self.get_error_response(
                        ...
                        self.log_error(
                            f"Flight #{idx} unexpected error: {type(e).__name__}: {str(e)} | Input: '{flight.get('flight_date')}'"
                        )
                        return self.get_error_response(
                    ...
                    flight_segments.append(
                        {
                            "departure_station": str(flight["departure_station"])
                            .strip()
                            .upper(),
                            "arrival_station": str(flight["arrival_station"])
                            .strip()
                            .upper(),
                            "flight_date": flight_date_fmt,
                        }
                    )
            elif request.data.get("flight_date"):
                # Backward compatibility: single flight date format
                flight_date = request.data.get("flight_date")
                
                dep = (
                    request.data.get("departure_station")
                    or flight_booking.flying_from
                    or ""
                )
                arr = (
                    request.data.get("arrival_station")
                    or flight_booking.flying_to
                    or ""
                )
                ...
                # Debug logging
                self.log_info(
                    f"Single flight reschedule: date='{flight_date_str}' | type={type(flight_date).__name__} | len={len(flight_date_str)} | dep={dep} | arr={arr}"
                )
                ...
                try:
                    ...
                    flight_date_fmt = dt.strftime("%Y%m%d")
                    self.log_info(
                        f"Date parsed successfully: '{flight_date_str}' -> '{flight_date_fmt}'"
                    )

                except ValueError as e:
                    self.log_error(
                        f"Date parsing ValueError: {str(e)} | Input: '{flight_date_str}'"
                    )
                    return self.get_error_response(
                    ...
                    self.log_error(
                        f"Unexpected date parsing error: {type(e).__name__}: {str(e)} | Input: '{flight_date_str}' | Raw: {repr(flight_date)}"
                    )
                    return self.get_error_response(
                ...
                # Fallback: Extract flight segments from booking data
                self.log_info("No flight data in request, extracting from booking")
                
                # Extract onward flight from booking
                if flight_booking.departure_date and flight_booking.flying_from and flight_booking.flying_to:
                    try:
                        dep_date = flight_booking.departure_date
                        if isinstance(dep_date, datetime):
                            flight_date_fmt = dep_date.strftime("%Y%m%d")
                        else:
                            # If it's a string, try to parse it
                            dep_date_str = str(dep_date).strip()
                            if "-" in dep_date_str:
                                dt = datetime.strptime(dep_date_str, "%Y-%m-%d")
                            elif len(dep_date_str) == 8 and dep_date_str.isdigit():
                                dt = datetime.strptime(dep_date_str, "%Y%m%d")
                            else:
                                dt = datetime.strptime(dep_date_str.split()[0], "%Y-%m-%d")
                            flight_date_fmt = dt.strftime("%Y%m%d")
                        
                        flight_segments.append({
                            "departure_station": str(flight_booking.flying_from).strip().upper(),
                            "arrival_station": str(flight_booking.flying_to).strip().upper(),
                            "flight_date": flight_date_fmt,
                        })
                    except Exception as e:
                        self.log_error(f"Error extracting onward flight from booking: {str(e)}")
                
                # Extract return flight from booking (if round-trip)
                if (trip_type in ["R", "Y"] and 
                    flight_booking.return_date and 
                    flight_booking.return_from and 
                    flight_booking.return_to):
                    try:
                        ret_date = flight_booking.return_date
                        if isinstance(ret_date, datetime):
                            flight_date_fmt = ret_date.strftime("%Y%m%d")
                        else:
                            # If it's a string, try to parse it
                            ret_date_str = str(ret_date).strip()
                            if "-" in ret_date_str:
                                dt = datetime.strptime(ret_date_str, "%Y-%m-%d")
                            elif len(ret_date_str) == 8 and ret_date_str.isdigit():
                                dt = datetime.strptime(ret_date_str, "%Y%m%d")
                            else:
                                dt = datetime.strptime(ret_date_str.split()[0], "%Y-%m-%d")
                            flight_date_fmt = dt.strftime("%Y%m%d")
                        
                        flight_segments.append({
                            "departure_station": str(flight_booking.return_from).strip().upper(),
                            "arrival_station": str(flight_booking.return_to).strip().upper(),
                            "flight_date": flight_date_fmt,
                        })
                    except Exception as e:
                        self.log_error(f"Error extracting return flight from booking: {str(e)}")
                
                # If still no segments, try to extract from AirIQ response data
                if not flight_segments:
                    try:
                        ...
                        for itin in itineraries:
                            ...
                            for seg in segments:
                                origin = seg.get("origin", "").strip().upper()
                                dest = seg.get("destination", "").strip().upper()
                                if origin and dest:
                                    # Use departure_date for onward, return_date for return
                                    if itin.get("trip_type") == "O" and flight_booking.departure_date:
                                        try:
                                            dep_date = flight_booking.departure_date
                                            if isinstance(dep_date, datetime):
                                                flight_date_fmt = dep_date.strftime("%Y%m%d")
                                            else:
                                                dep_date_str = str(dep_date).strip().split()[0]
                                                dt = datetime.strptime(dep_date_str, "%Y-%m-%d")
                                                flight_date_fmt = dt.strftime("%Y%m%d")
                                            flight_segments.append({
                                            ...
                                            pass
                                    elif itin.get("trip_type") == "R" and flight_booking.return_date:
                                        try:
                                            ret_date = flight_booking.return_date
                                            if isinstance(ret_date, datetime):
                                                flight_date_fmt = ret_date.strftime("%Y%m%d")
                                            else:
                                                ret_date_str = str(ret_date).strip().split()[0]
                                                dt = datetime.strptime(ret_date_str, "%Y-%m-%d")
                                                flight_date_fmt = dt.strftime("%Y%m%d")
                                            flight_segments.append({
                                                "departure_station": origin,
                        ...
                        self.log_error(f"Error extracting flight segments from AirIQ response: {str(e)}")

            # Validate flight_segments before API call
            if not flight_segments:
                return self.get_error_response(
                    message="No valid flight segments to reschedule. Please provide AvailInfo, flights array, flight_date, or ensure booking has flight data",
                    status="error",
                    status_code=status.HTTP_400_BAD_REQUEST,
                )
            ...
            # If a specific airiq_pnr was provided in request, use it
            # (already extracted at the beginning, but ensure it's in the list)
            if airiq_pnr and airiq_pnr not in airiq_pnrs:
                # If the provided PNR is not in the list, use it as the primary PNR
                airiq_pnrs = [airiq_pnr]

            # Check if user wants to reschedule specific PNR(s) only
            pnr_index = request.data.get("pnr_index")  # 0-based index
            specific_airiq_pnr = request.data.get(
                "airiq_pnr"
            ) or request.data.get("AirIqPNR")  # Specific PNR to reschedule
            ...
            if pnr_index is not None:
                try:
                    ...
                    if 0 <= pnr_idx < len(airiq_pnrs):
                        ...
                        airline_pnrs = (
                            [airline_pnrs[pnr_idx]]
                            if pnr_idx < len(airline_pnrs)
                            else []
                        )
                ...
                try:
                    ...
                    airiq_pnrs = [airiq_pnrs[pnr_idx]]
                    airline_pnrs = (
                        [airline_pnrs[pnr_idx]] if pnr_idx < len(airline_pnrs) else []
                    )
            ...
            if has_multiple_pnrs and is_roundtrip:
                ...
                from apps.booking.utils.flight_booking_utils import (
                    process_multi_pnr_reschedule_availability,
                )
                ...
                if segments_by_pnr and len(segments_by_pnr) == len(airiq_pnrs):
                            ...
                            segment_flights.append(
                                {
                                    "departure_station": str(
                                        seg.get("departure_station", "")
                                    )
                                    .strip()
                                    .upper(),
                                    "arrival_station": str(
                                        seg.get("arrival_station", "")
                                    )
                                    .strip()
                                    .upper(),
                                    "flight_date": str(
                        ...
                        reschedule_requests.append(
                            {
                                "airiq_pnr": airiq_pnr,
                                "airline_pnr": (
                                    airline_pnrs[idx] if idx < len(airline_pnrs) else ""
                                ),
                    ...
                    from apps.booking.utils.flight_booking_utils import (
                        extract_route_info_from_airiq_response,
                    )

                    route_info = extract_route_info_from_airiq_response(flight_booking)

                    onward_origin = (
                        route_info.get("onward_origin", "").strip().upper()
                        or (flight_booking.flying_from or "").strip().upper()
                    )
                    onward_dest = (
                        route_info.get("onward_destination", "").strip().upper()
                        or (flight_booking.flying_to or "").strip().upper()
                    )
                    return_origin = (
                        route_info.get("return_origin", "").strip().upper()
                        or (flight_booking.return_from or "").strip().upper()
                    )
                    return_dest = (
                        route_info.get("return_destination", "").strip().upper()
                        or (flight_booking.return_to or "").strip().upper()
                    )

                    # Log extracted route info for debugging
                    self.log_info(
                        f"Extracted route info: onward={onward_origin}-{onward_dest}, return={return_origin}-{return_dest}"
                    )
                    self.log_info(
                        f"Route info from AirIQ: {route_info.get('itineraries', [])}"
                    )
                    ...
                    for idx, airiq_pnr in enumerate(airiq_pnrs):
                        ...
                        if idx < len(itineraries):
                            ...
                            for req_seg in flight_segments:
                                req_dep = (
                                    req_seg.get("departure_station", "").strip().upper()
                                )
                                req_arr = (
                                    req_seg.get("arrival_station", "").strip().upper()
                                )

                                # Check if this segment matches any segment in the itinerary
                                for itin_seg in itin_segments:
                                    itin_origin = (
                                        (itin_seg.get("origin", "") or "")
                                        .strip()
                                        .upper()
                                    )
                                    itin_dest = (
                                        (itin_seg.get("destination", "") or "")
                                        .strip()
                                        .upper()
                                    )
                        ...
                        if not segment_flights:
                            if idx == 0:
                                ...
                                for seg in flight_segments:
                                    dep = (
                                        seg.get("departure_station", "").strip().upper()
                                    )
                                ...
                                if not segment_flights:
                                    ...
                                    segment_flights = (
                                        flight_segments[:mid_point]
                                        if mid_point > 0
                                        else flight_segments
                                    )
                            else:
                        ...
                        seg_routes = [
                            f"{s.get('departure_station')}-{s.get('arrival_station')}"
                            for s in segment_flights
                        ]
                        self.log_info(
                            f"PNR {idx + 1} ({segment_trip_type}): {len(segment_flights)} segments - {seg_routes}"
                        )
                ...
                self.log_info(
                    f"Processing {len(reschedule_requests)} reschedule requests for {len(airiq_pnrs)} PNRs"
                )
                ...
                # Use the airiq_pnr we extracted (from request or booking)
                pnr_to_use = airiq_pnrs[0] if airiq_pnrs else airiq_pnr
                self.log_info(
                    f"Calling AirIQ reschedule_availability with: trip_type={trip_type}, airiq_pnr={pnr_to_use}, segments={flight_segments}"
                )

                resp = airiq_service.reschedule_availability(
                    trip_type=trip_type,
                    flight_segments=flight_segments,
                    airiq_pnr=pnr_to_use,
                    remarks=remarks,
                )
            ...
            self.log_error(
                f"AirIQ reschedule availability error for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
            ...
            self.log_error(
                f"Error in reschedule availability for booking {booking_id}: {str(e)}"
            )
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "flag": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=["CHECKFARE", "CONFIRM"],
                    default="CHECKFARE",
                    description="Flag to check fare or confirm reschedule. Default: CHECKFARE. When CONFIRM, payment_channel is required.",
                ),
                "payment_channel": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=["WALLET", "PHONE PAY"],
                    description="Required when flag is CONFIRM. Payment method to use.",
                ),
                "redirect_url": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Required when payment_channel is PHONE PAY. URL to redirect after payment.",
                ),
                "flight_details": openapi.Schema(
                    ...
                    properties={
                        ...
                        "trip_type": openapi.Schema(
                            type=openapi.TYPE_STRING, enum=["O", "R", "Y"]
                        ),
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path=r"(?P<booking_id>\d+)/reschedule/confirm",
    )
    def reschedule_confirm(self, request, booking_id=None):
        try:
            booking, flight_booking = self.get_flight_booking(booking_id)
            
            # Initialize total_payment to avoid "cannot access local variable" error
            ...
            # If specific airiq_pnr provided in request, use it
            if airiq_pnr and airiq_pnr not in airiq_pnrs:
                airiq_pnrs = [airiq_pnr]
            ...
            # Get Trackid: Check request first, then fallback to booking
            track_id = request.data.get("Trackid") or request.data.get("track_id") or request.data.get("TrackId")
            if not track_id:
                # Fallback to booking
                track_ids = flight_booking.airiq_track_ids or []
                if track_ids:
                    track_id = track_ids[0]
                elif flight_booking.airiq_track_id:
                    track_id = flight_booking.airiq_track_id

            # Get SegmentInfo: Check request first, then fallback to booking
            segment_info = request.data.get("SegmentInfo") or request.data.get("segment_info") or {}
            base_origin = segment_info.get("BaseOrigin") or segment_info.get("base_origin")
            base_destination = segment_info.get("BaseDestination") or segment_info.get("base_destination")
            trip_type = segment_info.get("TripType") or segment_info.get("trip_type")
            
            # Fallback to booking for SegmentInfo
            if not base_origin:
                base_origin = flight_booking.flying_from
            if not base_destination:
                base_destination = flight_booking.flying_to
            if not trip_type or trip_type not in ["O", "R", "Y"]:
                # Derive from booking
                trip_type = flight_booking.search_session_data.get("trip_type") or "O"
                if trip_type not in ["O", "R", "Y"]:
            ...
            # Get ContactNo: Check request first, then fallback to booking
            contact_no = request.data.get("ContactNo") or request.data.get("contact_no")
            if not contact_no:
                # Fallback to booking contact info
                if booking and booking.customer:
                    contact_no = booking.customer.phone or booking.customer.mobile
                elif booking and hasattr(booking, 'contact_phone'):
                    contact_no = booking.contact_phone

            # Get Remarks: Check request first, default to empty
            remarks = request.data.get("Remarks") or request.data.get("remarks", "")
            
            # Get Flag: Check request first, default to "CHECKFARE"
            flag = request.data.get("Flag") or request.data.get("flag", "CHECKFARE")
            
            # Validate payment_channel is required when Flag is CONFIRM
            if flag == "CONFIRM":
                ...
                # Validate redirect_url is required when payment_channel is PHONE PAY
                if payment_channel == "PHONE PAY":
                    redirect_url = request.data.get("redirect_url") or request.data.get("redirectUrl")
                    if not redirect_url:
                        return self.get_error_response(
                            message="redirect_url is required when payment_channel is PHONE PAY",
                            status="error",
                            status_code=status.HTTP_400_BAD_REQUEST,
                        )

            # Get ItineraryInfo: Check request first, then fallback to flight_details or booking
            itinerary_info = request.data.get("ItineraryInfo") or request.data.get("itinerary_info") or []
            flight_details_list = []
            flight_details = request.data.get("flight_details") or {}
            flight_details_list_input = request.data.get("flight_details_list") or []

            # If ItineraryInfo is provided in request, convert it to flight_details format
            if itinerary_info and isinstance(itinerary_info, list):
                for itin in itinerary_info:
                    flight_details_item = {
                        "segments": itin.get("FlightDetails") or itin.get("flight_details") or [],
                        "base_amount": itin.get("BaseAmount") or itin.get("base_amount") or "0",
                        "gross_amount": itin.get("GrossAmount") or itin.get("gross_amount") or "0",
                    }
                    # Extract origin/destination from first segment if available
                    if flight_details_item["segments"]:
                        first_seg = flight_details_item["segments"][0]
                        flight_details_item["origin"] = first_seg.get("Origin") or first_seg.get("origin") or base_origin
                        last_seg = flight_details_item["segments"][-1]
                        flight_details_item["destination"] = last_seg.get("Destination") or last_seg.get("destination") or base_destination
                    else:
                        flight_details_item["origin"] = base_origin
                        flight_details_item["destination"] = base_destination
                    flight_details_item["trip_type"] = trip_type
                    flight_details_list.append(flight_details_item)
            # Support both single flight_details and multiple segments (backward compatibility)
            elif flight_details_list_input:
                # Multiple segments (onward and return)
                if len(flight_details_list_input) != len(airiq_pnrs):
                    return self.get_error_response(
                        message=f"Number of flight_details ({len(flight_details_list_input)}) must match number of PNRs ({len(airiq_pnrs)})",
                        status="error",
                        status_code=status.HTTP_400_BAD_REQUEST,
                    )
                flight_details_list = flight_details_list_input
            elif flight_details:
                # Single segment (backward compatibility)
                flight_details_list = [flight_details]
            else:
                # Fallback: Try to extract from booking data
                self.log_info("No ItineraryInfo or flight_details in request, attempting to extract from booking")
                
                # Try to extract from booking's selected flight data or AirIQ response
                selected_flight_data = flight_booking.selected_flight_data or {}
                airiq_response = flight_booking.airiq_response_data or {}
                
                # Extract from selected_flight_data
                if selected_flight_data:
                    segments = selected_flight_data.get("segments") or []
                    if segments:
                        flight_details_item = {
                            "origin": base_origin or flight_booking.flying_from,
                            "destination": base_destination or flight_booking.flying_to,
                            "trip_type": trip_type,
                            "segments": segments,
                            "base_amount": str(selected_flight_data.get("base_amount", 0)),
                            "gross_amount": str(selected_flight_data.get("gross_amount", 0)),
                        }
                        flight_details_list.append(flight_details_item)
                
                # If still empty, try to build from booking's basic flight info
                if not flight_details_list and flight_booking.departure_date:
                    # Build minimal flight details from booking
                    flight_details_item = {
                        "origin": base_origin or flight_booking.flying_from or "",
                        "destination": base_destination or flight_booking.flying_to or "",
                        "trip_type": trip_type,
                        "segments": [],
                        "base_amount": "0",
                        "gross_amount": "0",
            ...
            if not flight_details_list:
                return self.get_error_response(
                    message="ItineraryInfo, flight_details, or flight_details_list is required, or booking must have flight data",
                    status="error",
                    status_code=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not contact_no:
                return self.get_error_response(
                    message="ContactNo is required in request or booking must have contact information",
                    status="error",
                    status_code=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not track_id:
                return self.get_error_response(
                    message="Trackid is required in request or booking must have track_id",
                    status="error",
                    status_code=status.HTTP_400_BAD_REQUEST,
                )
            ...
            specific_airiq_pnr = request.data.get(
                "airiq_pnr"
            )  # Specific PNR to reschedule

            # Filter PNRs if specific one requested
            original_airiq_pnrs = (
                airiq_pnrs.copy() if isinstance(airiq_pnrs, list) else list(airiq_pnrs)
            )
            original_airline_pnrs = (
                airline_pnrs.copy()
                if isinstance(airline_pnrs, list)
                else list(airline_pnrs)
            )
            ...
            if pnr_index is not None:
                ...
                try:
                    ...
                    airline_pnrs = (
                        [original_airline_pnrs[pnr_idx]]
                        if pnr_idx < len(original_airline_pnrs)
                        else []
                    )
                except ValueError:
                    return self.get_error_response(
            ...
            if has_multiple_pnrs and is_roundtrip:
                ...
                # Initialize variables to avoid "cannot access local variable" errors
                successful_fare_checks = []
                failed_fare_checks = []
                ...
                if not track_ids:
                    if track_id:
                        ...
                        # Fallback: try to get from booking's track_ids
                        track_ids = flight_booking.airiq_track_ids or []
                        if not track_ids and flight_booking.airiq_track_id:
                            track_ids = [flight_booking.airiq_track_id] * len(airiq_pnrs)
                        if not track_ids:
                            return self.get_error_response(
                                message="Trackid or track_ids is required in request or booking must have track_id",
                                status="error",
                                status_code=status.HTTP_400_BAD_REQUEST,
                            )
                ...
                for resp_item in successful_fare_checks:
                    ...
                    total_fare_difference += Decimal(
                        str(resp_item.get("fare_difference", 0))
                    )
                ...
                if total_payment > 0:
                    # payment_channel should already be validated above for CONFIRM flag
                    payment_channel = request.data.get("payment_channel")
                    if not payment_channel:
                        return self.get_error_response(
                            message="payment_channel is required when Flag is CONFIRM. Use WALLET, PHONE PAY, or RAZORPAY",
                    ...
                    # Validate redirect_url for PhonePe
                    if payment_channel == "PHONE PAY":
                        redirect_url = request.data.get("redirect_url") or request.data.get("redirectUrl")
                        if not redirect_url:
                            return self.get_error_response(
                    ...
                    from apps.booking.utils.flight_payment_utils import (
                        handle_reschedule_wallet_payment,
                ...
                #     # Ensure successful_fare_checks is defined (defensive check)
                #     if 'successful_fare_checks' not in locals() or not successful_fare_checks:
                #         successful_fare_checks = []
                #     successful_requests = []
                #     for resp_item in successful_fare_checks:
                ...
                #     from apps.booking.utils.flight_booking_utils import (
                #         process_reschedule_success,
                #         process_reschedule_refund,
                #         record_reschedule_failure,
                #     )

                #     # Update booking for each successful reschedule
                #     all_new_pnrs = (
                #         original_airiq_pnrs.copy()
                #         if isinstance(original_airiq_pnrs, list)
                #         else list(original_airiq_pnrs)
                #     )
                ...
                #     refund_result = process_reschedule_refund(
                #         booking, refund_amount, reschedule_details
                #     )
                ...
                #     else:
                #         self.log_error(
                #             f"Reschedule successful but refund failed for booking {booking_id}: {refund_result.get('error')}"
                #         )
                #         response_data["refund"] = {
                #             "success": False,
                ...
                # Single PNR case (backward compatibility)
                # Use the track_id we extracted earlier (from request or booking)
                if not track_id:
                    return self.get_error_response(
                        message="Trackid is required in request or booking must have track_id",
                        status="error",
                ...
                # Ensure flight_details has required fields with fallback to extracted SegmentInfo
                if not flight_details.get("origin"):
                    flight_details["origin"] = base_origin
                if not flight_details.get("destination"):
                    flight_details["destination"] = base_destination
                if not flight_details.get("trip_type"):
                    flight_details["trip_type"] = trip_type
                ...
                fare_difference = Decimal(
                    str(check_fare_resp.get("FareDifference", 0) or 0)
                )
                ...
                if total_payment > 0:
                    ...
                    reschedule_details = {
                        ...
                        "old_booking_amount": float(
                            check_fare_resp.get("OldBookingAmount", 0) or 0
                        ),
                        "new_booking_amount": float(
                            check_fare_resp.get("NewBookingAmount", 0) or 0
                        ),
            ...
            print("Error in reschedule confirm:", str(e))
            self.log_error(
                f"Error in reschedule confirm for booking {booking_id}: {str(e)}"
            )
            return self.get_error_response(
                message="An unexpected error occurred",
                status="error",
