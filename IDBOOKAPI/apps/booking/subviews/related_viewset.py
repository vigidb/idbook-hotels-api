...
from apps.booking.utils.db_utils import (
    get_overall_booking_rating,
    check_review_exist_for_booking,
...
class ReviewViewSet(viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if review_exist:
            custom_response = self.get_error_response(
                message="Review already done for the booking",
                status="error",
                errors=[],
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="overall-rating/property",
        url_name="hold",
    ...
    def property_overall_rating(self, request):
        ...
        property_rating_list, agency_review_list = get_overall_booking_rating(
            property_id
        )
        data = {
            "property_overall_rating": property_rating_list,
...
class InvoiceViewSet(viewsets.ModelViewSet, StandardResponseMixin, LoggingMixin):
    ...
    @action(
        detail=False, methods=["GET"], url_path="summary", permission_classes=[AllowAny]
    )
    def summary(self, request):
        ...
        if invoice_ids:
            ...
            payments_received = (
                pay_qs.exclude(transaction_for="booking_refund").aggregate(
                    total=Sum("amount")
                )["total"]
                or 0
            )
            refunds_amount = (
                pay_qs.filter(transaction_for="booking_refund").aggregate(
                    total=Sum("amount")
                )["total"]
                or 0
            )
        ...
        end_date_str = request.query_params.get("end_date") or request.query_params.get(
            "to_date"
        )
        ...
        if not start_dt or not end_dt:
            bounds = qs.aggregate(
                min_date=Min("invoice_date"), max_date=Max("invoice_date")
            )
        ...
        series_map = {
            row["period"].date() if hasattr(row["period"], "date") else row["period"]: {
                k: row[k]
                for k in ("invoices", "total_amount", "gst_amount", "discount_amount")
            }
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        try:
            instance = (
                self.get_object()
            )  # This might raise ValueError if the invoice is not found
        ...
        if (
            "invoice_number" in request.data
            and request.data["invoice_number"] != instance.invoice_number
        ):
            return self.get_error_response(
        ...
        )  # Use existing items if not updating them
        total = 0
        total_tax = 0
        total_amount = 0
        ...
        additional_options = (
            data.get("additional_options") or instance.additional_options
        )
        discount_value = (
            additional_options.get("discountValue") if additional_options else None
        )
        discount_type = (
            additional_options.get("discountType") if additional_options else None
        )
        final_discount = 0

        if (
            additional_options
            and additional_options.get("totalDiscount")
            and discount_value
        ):
            try:
                discount_value = float(discount_value)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        for booking in bookings:
            ...
            payment_details = BookingPaymentDetail.objects.filter(
                booking=booking, invoice=instance
            )
    ...
    @action(
        detail=False,
        methods=["POST"],
        url_path="generate-invoice-number",
        permission_classes=[IsAuthenticated],
    )
    def generate_invoice_number(self, request):
        ...
        if not billed_by_id:
            ...
            if active_business:
                ...
                print(
                    f"DEBUG generate_invoice_number: Using active business ID = {billed_by_id}"
                )

        invoice_number = create_invoice_number(billed_by_id=billed_by_id)
        print(
            f"DEBUG generate_invoice_number: Generated invoice_number = {invoice_number}"
        )
