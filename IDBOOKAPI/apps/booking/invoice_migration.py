    ...
    try:
        ...
        if invoice_obj is None:
            try:
                ...
                print(
                    f"Error: Invoice with number {invoice_data['invoiceNumber']} not found in database"
                )
                return None
        else:
            invoice = invoice_obj
...
def validate_invoices():
    ...
    if not response_json.get("success") or not isinstance(
    ...
    try:
        billed_by = BusinessDetail.objects.get(
            business_name="Idbook hospitality private limited"
        )
        ...
        print(
            "Multiple businesses with name 'Idbook hospitality private limited' found"
        )
        billed_by = BusinessDetail.objects.filter(
            business_name="Idbook hospitality private limited"
        ).first()
    ...
    for invoice_data in invoices_data:
        try:
            ...
            try:
                if invoice_data.get("invoiceDate"):
                    invoice_date = datetime.strptime(
                        invoice_data.get("invoiceDate"), "%Y-%m-%dT%H:%M:%S.%fZ"
                    ).date()

                if invoice_data.get("dueDate"):
                    due_date = datetime.strptime(
                        invoice_data.get("dueDate"), "%Y-%m-%dT%H:%M:%S.%fZ"
                    ).date()
            ...
            print(
                f"Validation failed for invoice {invoice_data.get('invoiceNumber', 'unknown')}: {str(e)}"
            )
            failed_count += 1
...
def sync_invoices(limit=None):
    ...
    try:
        ...
        print(f"Failed to connect to the API: {str(e)}")
        return {"total": 0, "created": 0, "failed": 0, "skipped": 0, "error": str(e)}
    ...
    for invoice_data in invoices_data:
        ...
        try:
            ...
            invoice = Invoice(
                ...
                tags=(
                    ",".join(invoice_data.get("tags", []))
                    if isinstance(invoice_data.get("tags"), list)
                    else ""
                ),
                discount=0,
                reference="Other",
            ...
            try:
                pdf_url = migrated_invoice_pdf_generation(
                    invoice_data, invoice_obj=invoice, booking_id=None
                )
                if pdf_url:
                ...
                print(
                    f"Error generating PDF for invoice {invoice_number}: {str(pdf_error)}"
                )

        except Exception as e:
            print(
                f"Failed to create invoice {invoice_data.get('invoiceNumber', 'unknown')}: {str(e)}"
            )
            failed_count += 1
