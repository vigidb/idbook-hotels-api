...
from apps.org_resources.serializers import (
    CompanyDetailSerializer,
    UserSubscriptionProfileSerializer,
)
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, validated_data):
        ...
        ##                is_email_valid = email_validation(username)
            ...
            if email:
                user = EmailPasswordAuthBackend().authenticate(
                    email=email, password=password
                )
            elif mobile_number:
                user = PhonePasswordAuthBackend().authenticate(
...
class UserListSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        if user_subscription:
            user_subscription_details = UserSubscriptionProfileSerializer(
                user_subscription
            )
...
class BilledUserSerializer(serializers.Serializer):
    ...
    def validate_user_group(self, value):
        ...
        if value not in valid_groups:
            raise serializers.ValidationError(
                f"Invalid user group. Must be one of: {', '.join(valid_groups)}"
            )
        return value
