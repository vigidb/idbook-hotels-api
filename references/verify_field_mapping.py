...
def analyze_mapping():
    ...
    flight_item = {
        "FlightDetails": AIRIQ_SAMPLE["FlightDetails"],
        "Fares": AIRIQ_SAMPLE["Fares"],
    }
    ...
    print(
        f"✅ Airline Code: '{first_segment.get('AirlineCode', '')}' → '{airline_code}'"
    )
    print(
        f"✅ Refundable: '{first_segment.get('Refundable', '')}' → {first_segment.get('Refundable', 'False').lower() == 'true'}"
    )
    print(
        f"✅ Duration: '{first_segment.get('JourneyTime', '')}' → {first_segment.get('JourneyTime', '')} minutes"
    )
